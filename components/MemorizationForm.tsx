...
export const MemorizationForm: React.FC<MemorizationFormProps> = ({ students, actor, onSuccess, initialStudentId, onCancel }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-full">
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6 overflow-y-auto flex-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        {/* Row 1: Santri & Type */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">Nama Santri</label>
            {initialStudentId ? (
              <div className="w-full rounded-lg border border-slate-200 bg-slate-50 py-2.5 px-3 text-slate-600 font-medium capitalize">{selectedStudentData?.full_name || "Santri tidak ditemukan"}</div>
            ) : (
              <select
                required
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              >
                <input type="radio" name="status" value={option.val} checked={status === option.val} onChange={() => setStatus(option.val)} className="absolute opacity-0 w-0 h-0" />
        ...
        <div className="pt-6 border-t border-slate-100 flex justify-end">
          <Button type="submit" disabled={!selectedStudent || !surah || !selectedJuz}>
