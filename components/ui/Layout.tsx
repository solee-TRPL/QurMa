...
export const Layout: React.FC<LayoutProps> = ({ children, user, tenant, title, currentPage, onNavigate, onLogout, onAddAccount, onSwitchAccount, onRemoveAccount, onSyncAccounts, isImpersonating = false, onStopImpersonating, subtitle }) => {
  ...
  const isTodayHoliday = React.useMemo(() => {
    if (!tenant?.cycle_config?.activeDays) return false;
    const today = new Date();
    const day = today.getDay();
    let activeDays: number[] = [];
    try {
      activeDays = typeof tenant.cycle_config.activeDays === "string" ? JSON.parse(tenant.cycle_config.activeDays) : tenant.cycle_config.activeDays;
    } catch {
      return false;
    }
  ...
  const fetchMyNotifications = async () => {
    try {
      ...
      setNotifications(data.filter((n) => !n.message.toLowerCase().includes("undefined")));
    } catch (err) {}
  };
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (!newNotif.message.toLowerCase().includes("undefined")) {
  ...
  const renderFormattedMessage = (msg: string) => {
    return msg.split(" ").map((word, i) => {
      ...
      return highlight ? (
        <span key={i} className="font-extrabold text-slate-800">
          {word}{" "}
        </span>
      ) : (
        word + " "
  ...
  const NavItem = ({
    ...
    <div className={`px-3 py-0.5 ${isSubItem ? "pl-6" : ""}`}>
      ...
      >
        ...
        {active && !hasSubMenu && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-slate-900 animate-pulse" />}
        {hasSubMenu && <ChevronDown className={`w-3.5 h-3.5 ml-auto transition-transform ${isSubMenuOpen ? "rotate-180 text-slate-800" : "text-[#a5d1bd] group-hover:text-white"}`} />}
      </button>
    </div>
  );
  ...
  const getAvatarColor = (name: string) => {
    const colors = ["bg-emerald-500", "bg-blue-500", "bg-purple-500", "bg-amber-500", "bg-rose-500", "bg-primary-500", "bg-cyan-500"];
    let hash = 0;
  ...
  return (
    <div className={`min-h-screen bg-white font-sans text-slate-800 ${paddingTopClass}`}>
      {isImpersonating && (
        <div className="fixed top-0 left-0 right-0 h-10 bg-slate-800 text-white z-2100 flex items-center justify-between px-4 shadow-md">
          <div className="flex items-center gap-1.5 md:gap-2 text-[10px] md:text-sm font-medium animate-pulse min-w-0 pr-2">
            <EyeOff className="w-3.5 h-3.5 md:w-4 md:h-4 text-orange-400 shrink-0" />
            <span className="truncate">
              Mode Penyamaran: <strong>{user.full_name}</strong> <span className="hidden md:inline">({user.role})</span>
            </span>
          </div>
          <button onClick={onStopImpersonating} className="bg-white text-slate-900 px-2 md:px-3 py-1 rounded text-[9px] md:text-xs font-bold hover:bg-orange-100 transition-colors tracking-wide whitespace-nowrap shrink-0">
            Kembali ke Superadmin
          </button>
        </div>
      )}

      {mounted && isSidebarOpen && <div className="fixed inset-0 z-1900 bg-slate-900/20 backdrop-blur-sm lg:hidden" onClick={() => setIsSidebarOpen(false)}></div>}

      <aside className={`fixed inset-y-0 left-0 z-2000 w-64 bg-jade-600 border-r border-jade-700 flex flex-col h-[calc(100vh)] transition-transform duration-300 ease-in-out lg:translate-x-0 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} ${isImpersonating ? "mt-10 h-[calc(100vh-40px)]" : ""}`} onClick={(e) => e.stopPropagation()}>
          ...
          <div className="overflow-hidden">
            <h1 className="text-sm font-black text-white tracking-tight leading-tight">
              {isSuperAdmin
                ? "PLATFORM HQ"
                : (() => {
                    const name = tenant?.name?.toUpperCase() || platformName.toUpperCase();
                    if (name.startsWith("PONPES "))
                      return (
                        <>
                          {"PONPES "}
                          <br />
                    ...
                    if (name.startsWith("PONDOK PESANTREN "))
                      return (
                        <>
                          {"PONDOK PESANTREN "}
                          <br />
            ...
            <p className={`text-[9px] font-black uppercase tracking-widest mt-1 opacity-60 ${isSuperAdmin ? "text-[#a5d1bd]" : "text-primary-500"}`}>{isSuperAdmin ? "Super Control" : "Tahfidz System"}</p>
          </div>
        </div>
            ...
            <>
              ...
              {(user.role === UserRole.ADMIN || user.role === UserRole.SUPERVISOR) && (
                <>
                  ...
                  {user.role === UserRole.ADMIN && <NavItem icon={Users} label="Manajemen User" page="users" active={currentPage === "users"} />}
                  ...
                  <NavItem icon={Calendar} label="Monitor Hafalan Harian" page="monitor-hafalan-harian" active={currentPage === "monitor-hafalan-harian"} />
                  ...
                  {user.role === UserRole.ADMIN && <NavItem icon={ShieldCheck} label="Audit Logs" page="audit-logs" active={currentPage === "audit-logs"} />}
                </>
              )}
        ...
        <div className="py-2 pb-4 lg:pb-4 border-t border-white/5 shrink-0 bg-transparent space-y-1">
          {isSuperAdmin ? (
            <>
      ...
      <div className={`lg:pl-64 flex flex-col min-h-screen transition-all duration-300`} onClick={() => { if (isSidebarOpen) setIsSidebarOpen(false); }}>
        <header className={`fixed right-0 left-0 lg:left-64 z-1000 h-16 bg-white/80 backdrop-blur-md border-b border-slate-100 flex items-center px-4 lg:px-8 shrink-0 transition-all shadow-sm gap-1.5 sm:gap-3 ${topOffsetClass}`} onClick={() => { if (isSidebarOpen) setIsSidebarOpen(false); }}>
          {/* Hamburger */}
          <button className="lg:hidden p-1.5 sm:p-2 -ml-1.5 sm:-ml-2 text-slate-500 hover:bg-slate-100 rounded-lg transition-colors order-1 shrink-0" onClick={(e) => { e.stopPropagation(); setIsSidebarOpen(true); }}>
          ...
          {/* HOLIDAY INDICATOR */}
          {isTodayHoliday && (
            <div className="flex bg-rose-50 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg sm:rounded-xl border-2 border-rose-100 animate-in fade-in duration-500 order-2 lg:order-4 shrink-0 mr-auto md:mr-0 ml-2 md:ml-0" title="Hari ini disetel sebagai hari libur (tidak aktif)">
              <div className="flex flex-col items-start md:items-center justify-center">
                <span className="text-[6px] sm:text-[7px] text-rose-500 uppercase tracking-[0.2em] font-black leading-none mb-1">Status Hari Ini</span>
                <span className="text-[8px] sm:text-[9px] font-black uppercase tracking-wide text-rose-600 leading-none flex items-center gap-1.5">
                  <div className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse" /> HARI LIBUR
                </span>
              </div>
            </div>
          )}
          ...
          {user.role === UserRole.SANTRI && (
            <div className="flex bg-white px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg sm:rounded-xl border-2 border-jade-100 animate-in fade-in slide-in-from-left-4 duration-500 order-2 lg:order-4 shrink-0">
              <div className="flex flex-col items-start justify-center">
                <span className="text-[6px] sm:text-[7px] text-jade-500 uppercase tracking-[0.2em] font-black leading-none mb-1">Hafalan</span>
                <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-wide text-jade-700 leading-none">
                  {totalHafalan || "0"} JUZ
                  {studentPageCount != null && <span className="text-jade-700 font-black"> {studentPageCount} Halaman</span>}
                </span>
              </div>
            </div>
          )}
          ...
          {user.role === UserRole.TEACHER && halaqahName && (
            <div className="flex bg-white px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg sm:rounded-xl border-2 border-jade-100 animate-in fade-in slide-in-from-right-4 duration-500 order-3 lg:order-5 shrink-0">
              <div className="flex flex-col items-start justify-center">
                <span className="text-[6px] sm:text-[7px] text-jade-500 uppercase tracking-[0.2em] font-black leading-none mb-1 whitespace-nowrap">Halaqah</span>
                <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-tight text-jade-700 leading-none whitespace-nowrap">{halaqahName}</span>
              </div>
            </div>
          )}
            ...
            <div className="relative order-4 lg:order-3 shrink-0" ref={notifRef}>
              <button
                ...
                className={`w-7 h-7 sm:w-9 sm:h-9 rounded-lg sm:rounded-xl flex items-center justify-center transition-all border-2 ${
                  isNotificationsOpen ? "bg-jade-50 text-jade-600 border-jade-100 shadow-inner" : "bg-white text-jade-600 border-slate-100 hover:bg-jade-50 hover:text-jade-700"
                }`}
              >
                  ...
                  <div className="max-h-90 overflow-y-auto scrollbar-hide bg-slate-50/30">
                    {notifications.length > 0 ? (
                      notifications.slice(0, 20).map((notif) => (
                        ...
                        >
                          {!notif.is_read && <div className="absolute left-1 top-1/2 -translate-y-1/2 w-1 h-3 bg-jade-500 rounded-full" />}
                          <div
                            className={`w-9 h-9 rounded-xl shrink-0 flex items-center justify-center transition-transform group-hover:scale-105 duration-300 border-2 ${
                              notif.type === "success" ? "bg-emerald-50 text-emerald-600 border-emerald-100" : notif.type === "error" ? "bg-rose-50 text-rose-600 border-rose-100" : "bg-jade-50 text-jade-600 border-jade-100"
                            }`}
                          >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between gap-2 mb-1">
                              ...
                              <span className="text-[7.5px] font-black text-slate-400 uppercase tracking-tighter shrink-0" suppressHydrationWarning>
                                {formatTime(notif.created_at)}
                              </span>
                            </div>
                            <div className="text-[9.5px] font-black text-slate-500 leading-tight opacity-80 uppercase tracking-tight">{renderFormattedMessage(notif.message)}</div>
                          </div>
          ...
          <div className="relative order-5 lg:order-7 shrink-0 ml-auto lg:ml-0" ref={dropdownRef}>
            <button onClick={(e) => { e.stopPropagation(); setIsProfileOpen(!isProfileOpen); setIsSidebarOpen(false); }} className="flex items-center gap-1.5 sm:gap-3 hover:bg-slate-200 p-1 sm:p-1.5 rounded-xl sm:rounded-2xl transition-all border border-transparent hover:border-slate-200">
              ...
              <div className="w-7 h-7 sm:w-9 sm:h-9 bg-slate-200 rounded-[10px] sm:rounded-2xl overflow-hidden border-2 border-white shadow-sm ring-1 ring-slate-100">
                {user.avatar_url ? (
                  ...
                  <img
                    src={`https://ui-avatars.com/api/?name=${encodeURIComponent(user.role === UserRole.SANTRI && user.student_name ? user.student_name : user.full_name || "User")}&background=${isSuperAdmin ? "1e293b" : "4f46e5"}&color=fff`}
                    alt=""
                  />
                )}
              </div>
                        ...
                        {savedAccounts.map((acc) => {
                          ...
                          return (
                            <div key={accId} className="flex items-center group/item hover:bg-slate-50 rounded-lg transition-all border border-transparent hover:border-slate-100 overflow-hidden">
                              <button onClick={() => handleSwitchAccount(acc)} className="flex-1 flex items-center px-2 py-1.5 text-left min-w-0">
                                <div className="flex items-center gap-2 min-w-0 w-full">
                                  <div className={`w-7 h-7 shrink-0 rounded-full ${avatarColor} text-white flex items-center justify-center text-[10px] font-bold shadow-sm`}>{(acc.profile.full_name || "U").charAt(0).toUpperCase()}</div>
                                  <div className="min-w-0 flex-1 pb-1">
                                    <p className="text-[11px] font-bold text-slate-700 truncate pr-1" title={acc.profile.full_name}>
                                      {acc.profile.full_name}
                                    </p>
                                    <p className="text-[9px] text-slate-400 font-medium uppercase truncate">{(acc.profile.role || "").replace("_", " ")}</p>
                                  </div>
                                </div>
                              </button>
                              <div className="shrink-0 w-8 flex justify-center">
                                <button onClick={(e) => handleRemoveAccount(e, accId, acc.profile.full_name)} className="p-1.5 group-hover/item:opacity-100 hover:text-red-500 text-red-300 transition-all">
        ...
        <main className="flex-1 p-4 pt-20 lg:p-8 lg:pt-24 lg:min-h-[calc(100vh-64px)] min-h-0 overflow-x-hidden">
          <div className="max-400 mx-auto">{children}</div>
        </main>
        ...
        <ConfirmModal
          ...
          message={
            <span>
              ...
              <br />
              <span className="mt-2 block text-[10px] font-bold text-slate-400 uppercase tracking-widest opacity-75">Anda harus login manual lagi untuk menambahkannya kembali.</span>
            </span>
          }
        />
      </div>
