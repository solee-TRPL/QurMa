...
export const RecentActivityModal: React.FC<RecentActivityModalProps> = ({ isOpen, onClose, records, students }) => {
    ...
    return (
        ...
        >
            <div 
                className="bg-white rounded-xl w-full max-w-sm overflow-hidden flex flex-col animate-in zoom-in-95 duration-200 border-2 border-slate-200"
                ...
                <div className="p-4 overflow-y-auto scrollbar-hide max-h-[60vh] bg-slate-50/30">
                    {records.length > 0 ? (
                        <div className="space-y-3">
                            {records
                                .filter(rec => {
                                    // Filter out 0 pages/lines for SABAQ and SABQI
                                ...
                                .map(rec => {
                                    ...
                                    return (
                                    <div key={rec.id} className="bg-white p-3.5 rounded-xl border-2 border-slate-200 group hover:border-slate-300 transition-all shadow-none">
                                        <div className="flex justify-between items-start mb-2">
                                            <div className="flex items-center gap-2">
                                                <div className={`w-2 h-2 rounded-full ${
                                                    rec.status === MemorizationStatus.LANCAR ? 'bg-emerald-500' : 
                                                    rec.status === MemorizationStatus.TIDAK_LANCAR ? 'bg-amber-500' : 'bg-rose-500'
                                                }`} />
                                                <p className="text-[10px] font-black text-slate-800 uppercase tracking-tight">{student?.full_name || 'SANTRI'}</p>
                                            </div>
                                            <span className="text-[8px] font-black text-slate-400 uppercase tracking-[0.1em]">{new Date(rec.record_date).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}</span>
                                        </div>

                                        <div className="flex flex-wrap items-center gap-2 bg-slate-50 p-2 rounded-lg border border-slate-100">
                                            <div className="text-[9px] font-black text-slate-600 uppercase tracking-widest flex-1 min-w-[80px]">
                                                {(() => {
                                            ...
                                            <div className="flex items-center gap-1.5 shrink-0">
                                                ...
                                                <span className={`text-[7px] font-black px-1.5 py-0.5 rounded border-2 uppercase tracking-widest ${
                                                    rec.status === MemorizationStatus.LANCAR ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 
                                                    rec.status === MemorizationStatus.TIDAK_LANCAR ? 'bg-amber-50 text-amber-600 border-amber-200' : 
