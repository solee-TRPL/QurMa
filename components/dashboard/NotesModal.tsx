                    ...
                    <div className="space-y-2.5">
                        {notes.length === 0 ? (
                            ...
                            return (
                                <div 
                                    key={note.id} 
                                    className={`flex flex-col gap-2 bg-white p-3.5 rounded-xl border-2 border-slate-200 hover:border-slate-300 transition-all relative overflow-hidden ${hasReply ? 'cursor-pointer' : ''}`}
                                    onClick={() => hasReply && toggleExpand(note.id)}
                                >
                                    <div className="flex items-start gap-3">
                                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center border-2 shrink-0 mt-0.5 ${
                                                ...
                                                <div className="flex items-center gap-1.5">
                                                    ...
                                                    {hasReply && (
                                                        <div className="flex items-center gap-1 bg-jade-50 px-1.5 py-0.5 rounded border border-jade-100">
                                                            ...
                                                            <span className="text-[6.5px] font-black text-jade-600 uppercase tracking-tighter">Terbalas</span>
                                                        </div>
                                                    )}
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {!hasReply ? (
                                                        <button 
                                                            ...
                                                            className={`p-1 rounded-lg transition-all border-2 ${replyingTo === note.id ? 'bg-jade-600 text-white border-jade-600' : 'bg-white text-slate-300 border-slate-200 hover:text-jade-600 hover:border-jade-200'}`}
                                                        >
                                                            <MessageCircle className="w-3.5 h-3.5" />
                                                        </button>
                                                    ) : (
                                                        <div className={`p-1 transition-transform duration-300 ${isExpanded ? 'rotate-180 bg-slate-50 rounded-lg' : ''}`}>
                                                            <ChevronDown className={`w-3.5 h-3.5 ${isExpanded ? 'text-slate-800' : 'text-slate-300'}`} strokeWidth={3} />
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
                                            <p className="text-[10px] font-black text-slate-800 uppercase tracking-tight leading-normal mb-1">{note.content}</p>
                                            <p className="text-[7.5px] font-black text-slate-400 uppercase tracking-widest opacity-60">Ustadz: {note.teacher_name || 'Admin'}</p>
                                        </div>
                                    </div>
                                    ...
                                    {replyingTo === note.id && (
                                        <div className="mt-2 animate-in slide-in-from-top-1 duration-200" onClick={e => e.stopPropagation()}>
                                            <textarea 
                                                ...
                                                className="w-full px-3 py-2 bg-slate-50 border-2 border-slate-200 rounded-xl text-[10px] font-black uppercase text-slate-700 outline-none focus:bg-white focus:border-jade-400 focus:ring-4 focus:ring-jade-50/50 min-h-[70px] resize-none"
                                            />
                                            <div className="flex justify-end gap-2 mt-2">
                                                <button 
                                                    ...
                                                    className="px-3 py-1.5 text-[8.5px] font-black text-slate-400 uppercase tracking-widest hover:text-slate-600 transition-all"
                                                >
                                                    Batal
                                                </button>
                                                <button 
