...
export const ExamScheduleModal: React.FC<ExamModalProps> = ({ isOpen, onClose, studentId }) => {
    ...
    return (
        ...
        >
            <div 
                className="bg-white rounded-[28px] shadow-2xl w-full max-w-sm overflow-hidden flex flex-col animate-in zoom-in-95 duration-200 border border-slate-100"
                                ...
                                <div key={exam.id} className="p-4 bg-slate-50 border border-slate-100 rounded-2xl flex items-center justify-between group hover:bg-white hover:shadow-md transition-all">
                                    <div className="flex items-center gap-4">
                                        <div className="w-10 h-10 bg-white rounded-xl shadow-sm border border-slate-100 flex flex-col items-center justify-center shrink-0">
                                            ...
                                            <span className="text-[7px] font-black text-amber-400 uppercase leading-none mt-1">{new Date(exam.date).toLocaleDateString('id-ID', { month: 'short' })}</span>
                                        </div>
                                        <div>
                                    ...
                                    <span className={`px-2 py-0.5 rounded-lg text-[8px] font-black uppercase tracking-widest ${
                                        exam.status === 'upcoming' ? 'bg-amber-50 text-amber-600 border border-amber-100' : 'bg-emerald-50 text-emerald-600 border border-emerald-100'
