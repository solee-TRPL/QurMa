...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
    ...
    const [activeTab, setActiveTab] = useState<'Data Sekolah' | 'Hafalan' | 'Kehadiran'>('Data Sekolah');

    return (
        <div className="flex-1 h-auto flex flex-col gap-3 lg:gap-4 animate-fade-in pb-20 lg:pb-8">
            <div className="flex justify-end shrink-0">
                <div className="flex gap-2 p-1.5 bg-white border-[1.5px] border-slate-200/80 rounded-xl overflow-x-auto scrollbar-hide shadow-sm">
                    {['Data Sekolah', 'Hafalan', 'Kehadiran'].map((tab) => (
                        <button
                            key={tab}
                            onClick={() => setActiveTab(tab as any)}
                            className={`py-2 px-3 rounded-lg text-[9px] lg:text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${
                                activeTab === tab 
                                ? 'bg-[#226649] text-white shadow-sm' 
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 lg:gap-5 flex-1 min-h-0">
                ...
                <div className="lg:col-span-4 bg-white rounded-xl border-2 border-slate-300 p-5 lg:p-6 flex flex-col min-h-100 lg:min-h-85 relative transition-all duration-500 overflow-hidden shadow-none">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 shrink-0 gap-4">
                        ...
                        <div className="flex flex-wrap items-center gap-2 lg:gap-3 justify-start md:justify-end w-full lg:w-auto">
                            <div className="flex items-center bg-white px-3 lg:px-4 rounded-xl border-2 border-slate-300 shadow-none ring-1 ring-white h-9 shrink-0 gap-3 lg:gap-4">
                                ...
                                <div className="flex items-center gap-1.5 border-l border-slate-200 pl-3 lg:pl-4">
                            ...
                            <div className="flex gap-1 p-1 bg-white border-[1.5px] border-slate-200/80 rounded-xl shadow-sm shrink-0 items-center h-9">
                                {[
                                    { label: 'SABAQ', value: MemorizationType.SABAQ },
                                    { label: 'SABQI', value: MemorizationType.SABQI },
                                    { label: 'MANZIL', value: MemorizationType.MANZIL }
                                ].map((type) => (
                                    <button
                                        key={type.value}
                                        onClick={() => setAdminTrendType(type.value as any)}
                                        className={`py-1 px-3 lg:px-4 rounded-lg text-[8px] lg:text-[9px] font-black uppercase tracking-widest transition-all whitespace-nowrap h-full flex items-center justify-center ${
                                            adminTrendType === type.value || (adminTrendType === 'all' && type.value === MemorizationType.SABAQ)
                                            ? 'bg-[#226649] text-white shadow-sm' 
                                            : 'text-slate-400 hover:text-slate-600 bg-transparent'
                                        }`}
                                    >
                            ...
                            <div className="flex items-center bg-white px-3 lg:px-4 rounded-xl border-2 border-slate-300 shadow-none ring-1 ring-white h-9 gap-1.5 hover:border-jade-300 transition-all group cursor-pointer relative shrink-0">
                                <select 
                                    value={adminTrendPeriod} 
                                    onChange={(e) => setAdminTrendPeriod(e.target.value as any)}
                                    className="bg-transparent text-[8px] font-black text-slate-600 focus:outline-none cursor-pointer uppercase appearance-none pr-4 w-full h-full"
                                >
                            ...
                            <div className="flex items-center gap-1.5 h-9 shrink-0">
                                <div className="flex items-center gap-1 bg-[#226649] p-1 rounded-xl shadow-none h-full">
                                    <button 
                                        type="button"
                                        onClick={() => {
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 shrink-0 gap-4">
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
                        ...
                        <div className="flex flex-row items-center gap-2 w-full sm:w-auto">
                            <div className="flex items-center bg-white px-3.5 rounded-xl border-2 border-slate-300 shadow-none ring-1 ring-white h-9 lg:h-10 gap-2 hover:border-jade-300 transition-all group cursor-pointer relative flex-1 sm:flex-none sm:min-27.5">
                            ...
                            <div className="flex items-center gap-1 bg-jade-600 p-1 rounded-xl shadow-none flex-1 sm:flex-none sm:min-35 h-9 lg:h-10">
        ...
        <div className="flex flex-col gap-4">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-3.5 shrink-0">
                {[
                    { label: 'Hadir (H)', value: adminStats?.kehadiranToday?.hadir || 0, icon: Activity, color: 'jade', subtext: `${adminStats?.totalStudents ? Math.round(((adminStats?.kehadiranToday?.hadir || 0) / adminStats.totalStudents) * 100) : 0}% dari total` },
                    { label: 'Sakit (S)', value: adminStats?.kehadiranToday?.sakit || 0, icon: AlertCircle, color: 'amber', subtext: `${adminStats?.totalStudents ? Math.round(((adminStats?.kehadiranToday?.sakit || 0) / adminStats.totalStudents) * 100) : 0}% dari total` },
                    { label: 'Izin (I)', value: adminStats?.kehadiranToday?.izin || 0, icon: FileText, color: 'blue', subtext: `${adminStats?.totalStudents ? Math.round(((adminStats?.kehadiranToday?.izin || 0) / adminStats.totalStudents) * 100) : 0}% dari total` },
                    { label: 'Alpa (A)', value: adminStats?.kehadiranToday?.alpa || 0, icon: Target, color: 'rose', subtext: `${adminStats?.totalStudents ? Math.round(((adminStats?.kehadiranToday?.alpa || 0) / adminStats.totalStudents) * 100) : 0}% dari total` }
                ].map((stat, i) => (
            ...
            <div className="bg-white rounded-xl border-2 border-slate-300 p-5 lg:p-6 flex flex-col relative min-h75 transition-all duration-500 shadow-none">
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 shrink-0 gap-4">
                    ...
                    <div className="flex bg-slate-800 rounded-xl p-1 shrink-0 w-full lg:w-auto h-8 lg:h-9">
                        <button type="button" onClick={() => setAdminTargetWeekOffset(prev => prev - 1)} className="w-6 h-6 lg:w-7 lg:h-7 flex items-center justify-center bg-white/10 hover:bg-white/20 rounded-lg transition-all text-white"><ChevronLeft className="w-2.5 h-2.5" /></button>
                        <div className="flex-1 px-1 text-[6.5px] lg:text-[7.5px] font-black uppercase tracking-widest text-white flex items-center justify-center gap-1 lg:gap-1.5"><Calendar className="w-2.5 h-2.5 opacity-60 hidden xs:block" /><span className="whitespace-nowrap">{adminTargetWeekRange.display}</span></div>
                        <button type="button" onClick={() => setAdminTargetWeekOffset(prev => prev + 1)} className="w-6 h-6 lg:w-7 lg:h-7 flex items-center justify-center bg-white/10 hover:bg-white/20 rounded-lg transition-all text-white"><ChevronRight className="w-2.5 h-2.5" /></button>
                    </div>
