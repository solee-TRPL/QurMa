...
export const createUser = async (userData: any, actor: UserProfile): Promise<UserProfile> => {
  ...
  const isAlreadyRegistered = error?.message?.toLowerCase().includes("user already registered") || error?.message?.toLowerCase().includes("already registered");
  ...
  if (isAlreadyRegistered) {
    ...
    let { data: existingProfile } = await supabase.from("profiles").select("*").eq("email", userData.email).maybeSingle();
    ...
    await logAudit(actor, "CREATE", `User: ${existingProfile.full_name}`, `Orang tua lintas-sekolah: profil yang ada digunakan.`);
