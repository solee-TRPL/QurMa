...
import { MemorizationRecord, MemorizationType, MemorizationStatus, UserProfile, WeeklyTarget } from "../../types";
import { logAudit } from "./auditService";
import { createNotification } from "./notificationService";

export const getStudentRecords = async (studentId: string): Promise<MemorizationRecord[]> => {
  const supabase = getSupabase();
  const { data, error } = await supabase.from("memorization_records").select("*").eq("student_id", studentId).order("record_date", { ascending: false }).order("created_at", { ascending: false });
...
export const getLastProgressByType = async (studentId: string, untilDate?: string): Promise<Record<string, MemorizationRecord | null>> => {
  const supabase = getSupabase();
  let query = supabase.from("memorization_records").select("*").eq("student_id", studentId);
  ...
  results.sabaq = records.find((r) => r.type === MemorizationType.SABAQ && (r.status === MemorizationStatus.LANCAR || r.status === MemorizationStatus.HAFALAN_AWAL)) || null;
  results.sabqi = records.find((r) => r.type === MemorizationType.SABQI && (r.status === MemorizationStatus.LANCAR || r.status === MemorizationStatus.HAFALAN_AWAL)) || null;
  results.manzil = records.find((r) => r.type === MemorizationType.MANZIL && (r.status === MemorizationStatus.LANCAR || r.status === MemorizationStatus.HAFALAN_AWAL)) || null;

  return results;
};

export const getAbsoluteLastProgressByType = async (studentId: string, untilDate?: string): Promise<Record<string, MemorizationRecord | null>> => {
  const supabase = getSupabase();
  let query = supabase.from("memorization_records").select("*").eq("student_id", studentId);
...
export const getFirstRecordDate = async (studentId: string): Promise<string | null> => {
  const supabase = getSupabase();
  const { data, error } = await supabase.from("memorization_records").select("record_date").eq("student_id", studentId).order("record_date", { ascending: true }).limit(1).maybeSingle();

  if (error || !data) return null;
  return data.record_date;
};

export const checkHasInitialSabaq = async (studentId: string): Promise<boolean> => {
  const supabase = getSupabase();
  const { count, error } = await supabase
    .from("memorization_records")
    .select("id", { count: "exact", head: true })
    .eq("student_id", studentId)
    .in("status", [MemorizationStatus.LANCAR, MemorizationStatus.HAFALAN_AWAL]);
...
export const getHalaqahRecords = async (studentIds: string[]): Promise<MemorizationRecord[]> => {
  ...
  const { data, error } = await supabase.from("memorization_records").select("*").in("student_id", studentIds).order("record_date", { ascending: false });
  if (error || !data) return [];
  return data as MemorizationRecord[];
};
...
export const createRecord = async (record: Omit<MemorizationRecord, "id" | "created_at">, actor: UserProfile, studentName: string) => {
  ...
  try {
    ...
    const shouldNotify = record.status !== MemorizationStatus.HAFALAN_AWAL && record.status !== MemorizationStatus.EMPTY && record.jumlah != null;
...
export const deleteRecord = async (studentId: string, type: string, recordDate: string, actor: UserProfile, studentName: string) => {
  const supabase = getSupabase();
  const { error } = await supabase.from("memorization_records").delete().eq("student_id", studentId).eq("type", type).eq("record_date", recordDate);
  if (error) throw error;
...
export const getWeeklyMemorization = async (studentId: string, weekStart: string): Promise<any> => {
  const supabase = getSupabase();
  const { data, error } = await supabase.from("weekly_memorization").select("records_data").eq("student_id", studentId).eq("week_start", weekStart).maybeSingle();
...
export const upsertWeeklyMemorization = async (studentId: string, weekStart: string, recordsData: any, actor: UserProfile, studentName: string): Promise<void> => {
  const supabase = getSupabase();
  const { error } = await supabase.from("weekly_memorization").upsert(
    {
      tenant_id: actor.tenant_id,
...
export const getWeeklyAllTypeTotals = async (studentIds: string[], weekStart: string): Promise<Record<string, { sabaq: number; sabqi: number; manzil: number }>> => {
  ...
  for (let i = 0; i < studentIds.length; i += CHUNK_SIZE) {
    ...
    const { data, error } = await supabase.from("weekly_memorization").select("student_id, records_data").in("student_id", chunk).eq("week_start", weekStart);
...
export const getWeeklyAllTypeTotalsInRange = async (studentIds: string[], startDate: string, endDate: string): Promise<Record<string, { sabaq: number; sabqi: number; manzil: number }>> => {
  const supabase = getSupabase();
  const totals: Record<string, { sabaq: number; sabqi: number; manzil: number }> = {};
  if (studentIds.length === 0) return totals;
  ...
  for (let i = 0; i < studentIds.length; i += CHUNK_SIZE) {
    const chunk = studentIds.slice(i, i + CHUNK_SIZE);
    const { data, error } = await supabase.from("weekly_memorization").select("student_id, week_start, records_data").in("student_id", chunk).gte("week_start", startDate).lte("week_start", endDate);
    ...
    if (!error && data) {
      data.forEach((item) => {
        ...
        totals[`${item.student_id}_${item.week_start}`] = studentTotals;
      });
    }
  }
  return totals;
...
export const getWeeklyTargets = async (studentIds: string[], weekStart: string): Promise<WeeklyTarget[]> => {
  ...
  for (let i = 0; i < studentIds.length; i += CHUNK_SIZE) {
    const chunk = studentIds.slice(i, i + CHUNK_SIZE);
    const { data, error } = await supabase.from("weekly_targets").select("*").in("student_id", chunk).eq("week_start", weekStart);
...
export const getWeeklyTargetsInRange = async (studentIds: string[], startDate: string, endDate: string): Promise<WeeklyTarget[]> => {
  ...
  for (let i = 0; i < studentIds.length; i += CHUNK_SIZE) {
    const chunk = studentIds.slice(i, i + CHUNK_SIZE);
    const { data, error } = await supabase.from("weekly_targets").select("*").in("student_id", chunk).gte("week_start", startDate).lte("week_start", endDate).order("week_start", { ascending: false });
...
export const upsertWeeklyTarget = async (target: WeeklyTarget, actor: UserProfile, studentName: string): Promise<WeeklyTarget> => {
  const supabase = getSupabase();
  const { data, error } = await supabase.from("weekly_targets").upsert(target, { onConflict: "student_id,week_start" }).select().single();
  if (error) throw error;
...
export const getTenantRecords = async (studentIds: string[], startDate: string, endDate: string): Promise<MemorizationRecord[]> => {
  ...
  for (let i = 0; i < studentIds.length; i += CHUNK_SIZE) {
    const chunk = studentIds.slice(i, i + CHUNK_SIZE);
    const { data, error } = await supabase.from("memorization_records").select("*").in("student_id", chunk).gte("record_date", startDate).lte("record_date", endDate).order("record_date", { ascending: true });
...
export const getLatestMemorizationBaselines = async (studentIds: string[]): Promise<Record<string, Record<string, MemorizationRecord>>> => {
  ...
  const { data, error } = await supabase.from("memorization_records").select("*").in("student_id", studentIds).order("student_id").order("record_date", { ascending: false }).order("created_at", { ascending: false });

  if (error || !data) return {};

  const baselines: Record<string, Record<string, MemorizationRecord>> = {};
  data.forEach((rec) => {
    // Only consider records that actually have a surah_name and are LANCAR/HAFALAN_AWAL
    if (!rec.surah_name || (rec.status !== MemorizationStatus.LANCAR && rec.status !== MemorizationStatus.HAFALAN_AWAL)) return;
