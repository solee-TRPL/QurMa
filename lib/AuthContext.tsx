...
import { getTenant, getPlatformSettings } from "@/services/dataService";
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const [platformName, setPlatformName] = useState<string>("QurMa");
  ...
  const handleAuthSuccess = useCallback(
    async (userId: string, session: any) => {
  ...
  useEffect(() => {
    ...
    const cachedPlatformName = localStorage.getItem("qurma_platform_name") || "QurMa";
    if (cachedProfile) setUser(JSON.parse(cachedProfile));
    if (cachedTenant) setTenant(JSON.parse(cachedTenant));
    setPlatformName(cachedPlatformName);
    ...
    const initAuth = async () => {
      try {
        ...
        getPlatformSettings()
          .then((pSettings) => {
            if (pSettings?.platform_name) {
              setPlatformName(pSettings.platform_name);
              localStorage.setItem("qurma_platform_name", pSettings.platform_name);
            }
          })
          .catch(() => {});
  ...
  const proceedNavigation = () => {
    ...
    if (pendingPage) {
      ...
      switchAccount(pendingAccount, true);
      setPendingAccount(null);
    }
  };
  ...
  const switchAccount = async (targetAccount: any, force: boolean = false) => {
  ...
  const updateProfile = (p: UserProfile) => {
    setUser(p);
    localStorage.setItem("qurma_active_profile", JSON.stringify(p));
  };
  const updateTenant = (t: Tenant) => {
    setTenant(t);
    localStorage.setItem("qurma_active_tenant", JSON.stringify(t));
  };
  const updatePlatformName = (name: string) => {
    setPlatformName(name);
    localStorage.setItem("qurma_platform_name", name);
  };
  ...
  const value = {
    ...
    tenant,
    isInitializing,
    currentPage: (pathname?.split("/").pop() || "dashboard") as PageView,
    deviceId,
