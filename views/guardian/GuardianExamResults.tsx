  ...
  const getVerdictBadge = (verdict?: MemorizationStatus) => {
    ...
    switch (verdict) {
      case MemorizationStatus.LANCAR:
        return <span className="text-[10px] font-bold text-green-700 bg-green-50 px-2 py-0.5 rounded border border-green-100">Lancar</span>;
      case MemorizationStatus.TIDAK_LANCAR:
        return <span className="text-[10px] font-bold text-yellow-700 bg-yellow-50 px-2 py-0.5 rounded border border-yellow-100">Tidak Lancar</span>;
      case MemorizationStatus.TIDAK_SETOR:
        return <span className="text-[10px] font-bold text-red-700 bg-red-50 px-2 py-0.5 rounded border border-red-100">Tidak Setor</span>;
      default:
        return null;
    }
  };
            ...
            <div key={exam.id} className="border border-slate-200 rounded-lg p-4 bg-white hover:border-primary-200 transition-colors">
              <div className="flex justify-between items-start mb-2">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">{new Date(exam.date).toLocaleDateString("id-ID", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}</p>
                </div>
              ...
              <div className="flex items-center gap-3 text-xs text-slate-500 border-t border-slate-100 pt-3 mt-3">
                <span className="flex items-center">
                  <User className="w-3 h-3 mr-1" /> {exam.examiner_name}
                </span>
...
export const StudentExamResults: React.FC<{ user?: UserProfile }> = ({ user }) => {
  ...
  const getCompactVerdict = (verdict?: MemorizationStatus) => {
    ...
    switch (verdict) {
      case MemorizationStatus.LANCAR:
        return <span className="text-[10px] font-bold text-green-600 bg-green-50 px-2 py-0.5 rounded border border-green-100">Lancar</span>;
      case MemorizationStatus.TIDAK_LANCAR:
        return <span className="text-[10px] font-bold text-yellow-600 bg-yellow-50 px-2 py-0.5 rounded border border-yellow-100">Tidak Lancar</span>;
      case MemorizationStatus.TIDAK_SETOR:
        return <span className="text-[10px] font-bold text-red-600 bg-red-50 px-2 py-0.5 rounded border border-red-100">Tidak Setor</span>;
      default:
        return null;
    }
  };
  ...
  if (!studentName && !loading && exams.length === 0) {
    return (
      <div className="h-[calc(100vh-140px)] flex items-center justify-center animate-fade-in">
        <EmptyState message="Tidak ada santri terhubung." description="Silakan hubungi admin sekolah untuk menautkan akun Anda dengan data santri." icon="user" />
      </div>
    );
  }
  ...
  return (
    <div className="h-[calc(100vh-160px)] flex flex-col items-center justify-center animate-fade-in relative overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-125 h-125 bg-jade-50/50 rounded-full blur-[120px] -z-10" />
      ...
      <div className="bg-white/40 backdrop-blur-md p-10 rounded-32px border border-white/20 shadow-xl shadow-primary-100/20 text-center max-w-sm relative z-10 transition-all hover:scale-[1.02] duration-500">
        <div className="w-20 h-20 bg-jade-600 rounded-3xl flex items-center justify-center mx-auto mb-8 shadow-lg shadow-primary-200 animate-bounce-slow">
          <Award className="w-10 h-10 text-white" />
        </div>

        <h2 className="text-xl font-black text-slate-800 uppercase tracking-[0.2em] mb-3">Nilai Ujian</h2>
        <div className="flex items-center justify-center gap-2 mb-6">
          <div className="h-0.625 w-6 bg-jade-100 rounded-full" />
          <p className="text-[10px] font-black text-jade-400 uppercase tracking-widest">Segera Hadir</p>
          <div className="h-0.625 w-6 bg-jade-100 rounded-full" />
        </div>

        <p className="text-xs font-bold text-slate-400 leading-relaxed uppercase tracking-tight">Fitur evaluasi hasil ujian kumulatif sedang dalam tahap sinkronisasi data kurikulum.</p>
      ...
      <p className="mt-12 text-[10px] font-black text-slate-300 uppercase tracking-[0.3em] animate-pulse">QurMa Administrative Intelligence</p>
    </div>
  );
};
