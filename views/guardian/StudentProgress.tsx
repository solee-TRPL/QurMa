  ...
  useEffect(() => {
    const fetchStudentAndProgress = async () => {
      ...
      try {
        ...
        if (myStudent) {
          const { data, error } = await supabase.from("student_semester_progress").select("progress_data").eq("student_id", myStudent.id).maybeSingle();
        ...
        <div className="flex items-center gap-4 w-full md:w-auto relative z-10">
          <div className="flex-1 md:flex-none bg-white/40 backdrop-blur-sm px-6 py-3 rounded-2xl border border-slate-200/50 text-center transition-all hover:bg-white hover:shadow-md hover:border-jade-100">
            ...
            <p className="text-lg font-black text-slate-800 leading-none">
              {maxSabaqActual} <span className="text-[10px] text-slate-300">Hal</span>
            </p>
          </div>
          <div className="flex-1 md:flex-none bg-jade-50/50 backdrop-blur-sm px-6 py-3 rounded-2xl border border-jade-100/30 text-center transition-all hover:bg-white hover:shadow-md hover:border-jade-200 group/btn">
            ...
            <p className="text-lg font-black text-jade-700 leading-none group-hover/btn:text-jade-800">
              {maxHafalanActual.toFixed(1)} <span className="text-[10px] text-jade-300">Juz</span>
            </p>
          </div>
        </div>
      </div>
            ...
            <tbody className="bg-white">
              {progressData.map((row, idx) => (
                <tr key={`${row.kelas}-${row.semester}`} className="hover:bg-slate-50/50 transition-colors group">
                  <td className={`px-3 py-1.5 border-r border-b border-slate-200 text-[10px] font-black text-slate-300 ${idx === progressData.length - 1 ? "border-b-0" : ""}`}>{String(idx + 1).padStart(2, "0")}</td>
                  {row.semester === 1 ? (
                  ...
                  <td className={`px-4 py-1.5 border-r border-b border-slate-200 text-center ${idx === progressData.length - 1 ? "border-b-0" : ""}`}>
                    {row.hafalanKet && (
                      <span
                        className={`px-2 py-0.5 rounded-xl text-[8px] font-black uppercase tracking-widest border shadow-sm ${row.hafalanKet === "Tercapai" ? "bg-emerald-50 text-emerald-600 border-emerald-100/50" : "bg-rose-50 text-rose-500 border-rose-100/50"}`}
                      >
                        {row.hafalanKet}
                      </span>
                    )}
                  ...
                  <td className={`px-4 py-1.5 border-b border-slate-200 text-center ${idx === progressData.length - 1 ? "border-b-0" : ""}`}>
                    {row.sabaqKet && (
                      <span
                        className={`px-2 py-0.5 rounded-xl text-[8px] font-black uppercase tracking-widest border shadow-sm ${row.sabaqKet === "Tercapai" ? "bg-emerald-50 text-emerald-600 border-emerald-100/50" : "bg-rose-50 text-rose-500 border-rose-100/50"}`}
                      >
                        {row.sabaqKet}
                      </span>
                    )}
