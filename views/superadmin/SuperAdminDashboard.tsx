...
export const SuperAdminDashboard: React.FC<SuperAdminDashboardProps> = ({ user, onNavigate }) => {
  ...
  const growthData = useMemo(() => {
    const last6Months = Array.from({ length: 6 }, (_, i) => {
      const d = new Date();
      d.setDate(1); // Set to 1st of the month to prevent month overflow bugs
  ...
  const recentTenants = useMemo(() => {
    return [...tenants].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).slice(0, 5);
  }, [tenants]);
      ...
      <div className="bg-white rounded-xl border-2 border-slate-300 p-3 lg:p-4 flex items-center gap-3 lg:gap-4 relative overflow-hidden group hover:border-jade-300 transition-all shadow-none">
        <div
          className={`absolute right-0 top-0 w-24 h-24 rounded-2xl -translate-y-8 translate-x-8 group-hover:scale-110 transition-transform duration-500 ${
            c === "jade" ? "bg-jade-50/40" : c === "blue" ? "bg-blue-50/40" : c === "orange" ? "bg-orange-50/40" : "bg-emerald-50/40"
          }`}
        />
        <div
        ...
        <div className="relative z-10">
          ...
          <h4 className="text-sm lg:text-lg font-black text-slate-800 leading-none">{loading ? <div className="h-4 lg:h-5 w-8 lg:w-12 bg-slate-100 animate-pulse rounded" /> : value}</h4>
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="lg:h-[calc(100vh-140px)] flex flex-col gap-4 animate-fade-in lg:overflow-hidden pb-0">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 min-h-0">
        {/* Growth Chart */}
        <div className="lg:col-span-3 bg-white p-5 lg:p-6 rounded-xl border-2 border-slate-300 shadow-none relative overflow-hidden flex flex-col min-h-62.5 md:min-h-75 lg:min-h-0">
          <div className="flex justify-between items-center mb-6 shrink-0">
            <div className="flex flex-col">
              <h3 className="text-[10px] font-black text-slate-800 uppercase tracking-[0.2em] mb-1">Pertumbuhan Sekolah</h3>
            ...
            {isMounted && (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={growthData} margin={{ top: 10, right: 10, left: -30, bottom: 0 }}>
                  ...
                  <YAxis axisLine={false} tickLine={false} tick={{ fill: "#94a3b8", fontSize: 8, fontWeight: 900 }} allowDecimals={false} />
                  <Tooltip
                  ...
                  />
                  <Area type="monotone" dataKey="total" stroke="var(--color-jade-600)" strokeWidth={4} fillOpacity={1} fill="url(#colorGrowth)" animationDuration={1500} />
                </AreaChart>
              </ResponsiveContainer>
            )}
          </div>
