...
export const TargetManagement: React.FC<{ tenantId: string; user: UserProfile }> = ({ tenantId, user }) => {
  ...
  useEffect(() => {
    const initTargets = async () => {
      try {
        const [classes, tenant] = await Promise.all([getClasses(tenantId), getTenant(tenantId)]);
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Two Column Layout: Table (Left) and Sidebar (Right) */}
                      ...
                      {sabaqTargets.map((st, i) => (
                        <tr key={i} className="group transition-colors hover:bg-slate-50/30">
                          ...
                          {st.total === null ? (
                            <td
                              colSpan={2}
                              className={`px-6 py-4 text-[10.5px] font-black text-slate-400 text-center bg-slate-50/5 border-b border-slate-100 uppercase tracking-widest ${!isReadOnly && "cursor-pointer hover:bg-slate-300"} transition-all`}
                              onClick={() => !isReadOnly && setEditingCell({ row: i, field: "sabaq-target" })}
                            >
                            ...
                            <>
                              <td
                                className={`px-6 py-4 text-[13px] text-jade-700 font-black text-center ${!isReadOnly && "cursor-pointer hover:bg-jade-50/30"} transition-all border-r border-b border-slate-100 bg-jade-50/5`}
                                onClick={() => !isReadOnly && setEditingCell({ row: i, field: "sabaq-target" })}
                              >
                                {editingCell?.row === i && editingCell?.field === "sabaq-target" ? (
                ...
                <div className="overflow-x-auto scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent">
                  <table className="min-w-full border-separate border-spacing-0">
                    ...
                    <tbody className="bg-white">
                      {manzilAcuan.map((m, i) => (
                        <tr key={i} className="group transition-colors hover:bg-slate-50/30">
                          <td className="px-4 py-4 whitespace-nowrap text-[15px] font-black text-slate-800 text-center border-r border-b border-slate-100 bg-slate-50/20 w-24">{m.id}</td>
                          <td
                            className={`px-6 py-4 text-[13px] text-slate-700 border-r border-b border-slate-100 text-center transition-all ${!isReadOnly && "cursor-pointer hover:bg-slate-300"}`}
                            onClick={() => !isReadOnly && setEditingCell({ row: i, field: "manzil-jumlah" })}
                          >
                          ...
                          <td
                            className={`px-6 py-4 text-[13px] text-jade-700 font-black text-center transition-all border-b border-slate-100 bg-jade-50/5 ${!isReadOnly && "cursor-pointer hover:bg-jade-50/30"}`}
                            onClick={() => !isReadOnly && setEditingCell({ row: i, field: "manzil-atm" })}
                          >
                  ...
                  <table className="min-w-full border-separate border-spacing-0">
                    ...
                    <tbody className="bg-white">
                      {sabqiAcuan.map((s, i) => (
                        <tr key={i} className="group transition-colors hover:bg-slate-50/30">
                          <td
                            className={`px-6 py-4 whitespace-nowrap text-[13px] font-black text-slate-700 border-r border-b border-slate-100 transition-all text-center bg-slate-50/20 ${!isReadOnly && "cursor-pointer hover:bg-jade-50/30"}`}
                            onClick={() => !isReadOnly && setEditingCell({ row: i, field: "sabqi-hafalan" })}
                          >
        ...
        <div className="lg:col-span-1 space-y-4 sticky top-6">
          <div className="bg-jade-600 rounded-28px p-6 shadow-xl shadow-primary-100 border border-jade-500 relative overflow-hidden group">
            ...
            <div className="relative z-10 space-y-4">
              ...
              <p className="text-[11px] text-white/90 font-bold leading-relaxed">Data target ini akan menjadi standar acuan sistem dalam mengukur ketercapaian hafalan santri di Dashboard Guru dan Laporan Wali Santri.</p>

              <div className="pt-2 bg-white/10 rounded-2xl p-4 border border-white/10 backdrop-blur-sm">
                <p className="text-[10px] text-white/80 font-bold italic leading-tight">"Klik pada teks di dalam tabel untuk menyesuaikan angka target dengan kebijakan sekolah secara instan."</p>
              </div>
            </div>
          </div>
