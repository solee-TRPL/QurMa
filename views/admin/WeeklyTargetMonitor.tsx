...
import { getHalaqahs, getStudents, getClasses, getWeeklyTargets, getWeeklyAllTypeTotals, getUsers, getTenant, updateTenant, upsertWeeklyTarget, createNotification } from "../../services/dataService";
import { ClipboardList, Calendar, Search, BookOpen, CheckCircle2, XCircle, TrendingUp, AlertCircle, GraduationCap, ChevronRight, ChevronLeft, Info, Target, User, ChevronDown, Filter, Check, Edit3, X } from "lucide-react";
...
export const WeeklyTargetMonitor: React.FC<WeeklyTargetMonitorProps> = ({ user, tenantId, showNotesMode = false, onNavigate }) => {
  ...
  const [selectedHalaqahId, setSelectedHalaqahId] = useState<string>(() => {
    ...
    return params.get("halaqahId") || localStorage.getItem("qurma_wtm_halaqahId") || "all";
  });
  ...
  const handleSaveSupervisorNote = async (targetRow: TargetRow, newNote: string) => {
    try {
      ...
      target.target_data = {
        ...(target.target_data || {}),
        supervisor_note: newNote,
      };
      await upsertWeeklyTarget(target, user, targetRow.name);

      if (newNote.trim().length > 0 && newNote !== targetRow.supervisorNote) {
  ...
  const performDataFetch = async (hList: Halaqah[]) => {
    ...
    try {
      ...
      const [studentData, classData] = await Promise.all([getStudents(tenantId), getClasses(tenantId)]);
  ...
  useEffect(() => {
    const initStatic = async () => {
      ...
      try {
        ...
        const [allUsers, tenantData, halaqahData] = await Promise.all([getUsers(tenantId), getTenant(tenantId), getHalaqahs(tenantId)]);
  ...
  const filteredStudents = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter((s) => s.full_name.toLowerCase().includes(query) || s.nis?.toLowerCase().includes(query));
    }
    ...
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="flex flex-col lg:flex-row items-center gap-2 lg:gap-2">
        ...
        <div className="flex flex-row items-center gap-2 w-full lg:flex-1">
          {/* Halaqah Selector */}
          <div className="flex-1 lg:flex-none flex items-center gap-2.5 md:gap-3 bg-white pt-2 md:pt-2 pb-2 md:pb-2 pl-4.5 pr-4.5 rounded-xl border-2 border-slate-300 shadow-none lg:min-w-85 focus-within:ring-4 focus-within:ring-jade-50/50 focus-within:border-jade-400 transition-all cursor-pointer" onClick={() => setShowHalaqahDropdown(!showHalaqahDropdown)}>
                ...
                <div className="flex items-center justify-between w-full pr-1">
                  <span className="bg-transparent text-[9px] md:text-[10px] font-black text-slate-800 uppercase tracking-tight truncate pointer-events-none leading-none">
                    {selectedHalaqahId === "all"
                      ? "SEMUA HALAQOH"
                      : (() => {
                          const h = allHalaqahs.find((h) => h.id === selectedHalaqahId);
                          return h ? `${h.name.toUpperCase()} / ${h.teacher_name?.toUpperCase() || "-"}` : "PILIH HALAQOH";
                        })()}
                  </span>
                  <ChevronDown className={`w-3 h-3 text-slate-300 pointer-events-none group-hover/sel-unified:text-jade-400 transition-all ${showHalaqahDropdown ? "rotate-180 text-jade-400" : ""}`} />
                </div>
                ...
                {showHalaqahDropdown && (
                  <>
                    ...
                    <div className="absolute top-[calc(100%+8px)] -left-4.5 -right-4.5 bg-white border-2 border-slate-300 rounded-xl shadow-none z-50 py-1 max-h-48 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent animate-in fade-in zoom-in-95 duration-200">
                      <div
                        className={`px-2 md:px-3 py-2 md:py-2.5 border-b border-slate-200 last:border-0 text-[8.5px] md:text-[10px] font-black tracking-tight md:tracking-widest cursor-pointer text-left hover:bg-slate-50 transition-colors truncate ${selectedHalaqahId === "all" ? "text-jade-600 bg-jade-50" : "text-slate-600"}`}
                        onClick={(e) => {
                          e.stopPropagation();
                          handleHalaqahChange("all");
                          setShowHalaqahDropdown(false);
                        }}
                        title="SEMUA HALAQOH"
                      >
                      ...
                      {allHalaqahs.map((h) => (
                        <div
                          key={h.id}
                          className={`px-2 md:px-3 py-2 md:py-2.5 border-b border-slate-200 last:border-0 text-[8.5px] md:text-[10px] font-black tracking-tight md:tracking-widest cursor-pointer text-left hover:bg-slate-50 transition-colors truncate ${selectedHalaqahId === h.id ? "text-jade-600 bg-jade-50" : "text-slate-600"}`}
                          onClick={(e) => {
                            e.stopPropagation();
                            handleHalaqahChange(h.id);
                            setShowHalaqahDropdown(false);
                          }}
        ...
        <div className="flex flex-row items-center gap-2 w-full lg:w-auto">
          ...
          <div className="flex-1 lg:flex-none flex bg-white p-1 rounded-xl border-2 border-slate-300 shadow-none h-10 lg:h-11 justify-center items-center">
            <button onClick={() => setCurrentWeekOffset((prev) => prev - 1)} className="p-1 px-2 hover:bg-slate-50 rounded-lg transition-colors text-slate-400">
      ...
      <div className="bg-transparent rounded-none overflow-hidden flex flex-col">
        <div className="px-1 pb-3 bg-transparent flex flex-row flex-wrap justify-between items-center w-full gap-2 transition-all">
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`flex-none flex items-center justify-center gap-1.5 px-2 md:px-3 py-1 md:py-1.5 rounded-lg md:rounded-xl transition-all text-[7.5px] md:text-[9.5px] font-black uppercase tracking-tight md:tracking-widest border-[1.5px] md:border-2 ${showNotes ? "bg-amber-50/50 text-amber-600 border-amber-300 shadow-none" : "bg-white text-slate-400 border-slate-300 hover:bg-slate-50"}`}
            >
          ...
          <div className="flex items-center justify-end gap-1.5 md:gap-4 overflow-x-auto no-scrollbar py-1 flex-1 min-w-0">
            <span className="flex items-center gap-1 text-[7px] md:text-[9px] font-black text-amber-500 uppercase tracking-tighter whitespace-nowrap">
              <div className="w-1.5 md:w-3 h-1.5 md:h-3 rounded-full border-[1.5px] md:border-2 border-amber-400/50 flex items-center justify-center">
            ...
            <span className="flex items-center gap-1 text-[7px] md:text-[9px] font-black text-emerald-600 uppercase tracking-tighter whitespace-nowrap">
              <div className="w-1.5 md:w-3 h-1.5 md:h-3 rounded-full border-[1.5px] md:border-2 border-emerald-400/50 flex items-center justify-center">
            ...
            <span className="flex items-center gap-1 text-[7px] md:text-[9px] font-black text-rose-500 uppercase tracking-tighter whitespace-nowrap">
              <div className="w-1.5 md:w-3 h-1.5 md:h-3 rounded-full border-[1.5px] md:border-2 border-rose-400/50 flex items-center justify-center">
            ...
            <table className="border-separate border-spacing-0" style={{ minWidth: '100%' }}>
              <thead className="sticky top-0 z-40">
                <tr className="bg-white">
                  <th
                    rowSpan={2}
                    style={{ position: 'sticky', left: 0, width: 36, minWidth: 36 }}
                    className="bg-slate-300 z-50 px-0.5 py-4 text-[8px] md:text-[9px] lg:text-[10px] font-black text-slate-800 uppercase tracking-widest text-center border-t border-b border-l border-black"
                  >
                    NO
                  </th>
                  {showNisKelas && (
                    <th
                      rowSpan={2}
                      style={{ position: 'sticky', left: 36, width: 76, minWidth: 76 }}
                      className="bg-slate-300 z-50 px-1 py-4 text-[9px] lg:text-[10px] font-black text-slate-800 uppercase tracking-widest text-center border-t border-b border-l border-black animate-in slide-in-from-left-1 duration-300"
                    >
                      NIS
                    </th>
                  )}
                  <th
                    ...
                    style={{
                      position: 'sticky',
                      left: showNisKelas ? 112 : 36,
                      width: 140,
                      minWidth: 140,
                    }}
                    className="bg-slate-300 z-50 px-2 py-4 text-[9px] lg:text-[10px] font-black text-slate-800 uppercase tracking-widest text-left border-t border-b border-r border-l border-black transition-all duration-300"
                  >
                    NAMA SANTRI
                  </th>
                  <th
                    rowSpan={2}
                    className="hidden lg:table-cell px-4 py-4 text-[9px] lg:text-[10px] font-black text-slate-800 uppercase tracking-widest text-center border-t border-b border-r border-black bg-slate-300 w-45 min-w-45 max-w-45"
                  >
                    HALAQOH ( PENGAMPU )
                  </th>
                  ...
                  {!showNotes ? (
                    <>
                      <th
                        colSpan={2}
                        className="px-4 py-3 text-[9px] lg:text-[10px] font-black text-emerald-700 uppercase tracking-widest border-t border-b border-r border-emerald-700 bg-emerald-50 shadow-inner whitespace-nowrap text-center"
                      >
                        HAFALAN SAAT INI
                      </th>
                      <th colSpan={3} className="px-4 py-3 text-[9px] lg:text-[10px] font-black text-blue-700 uppercase tracking-widest border-t border-b border-r border-blue-700 bg-blue-50 shadow-inner whitespace-nowrap text-center">
                        ATM
                      </th>
                      <th colSpan={6} className="px-4 py-3 text-[9px] lg:text-[10px] font-black text-emerald-600 uppercase tracking-widest text-center border-t border-b border-r border-emerald-600 bg-emerald-50/50 whitespace-nowrap">
                        TARGET PEKANAN
                      </th>
                    </>
                  ) : (
                    <th className="px-4 py-3 text-[9px] lg:text-[10px] font-black text-amber-600 uppercase tracking-widest border-t border-b border-r border-amber-600 bg-amber-50/30 text-left whitespace-nowrap">
                      CATATAN USTADZ & SUPERVISOR
                    </th>
                  )}
                </tr>
                {!showNotes ? (
                  ...
                  paginatedStudents.map((s, idx) => {
                    ...
                    return (
                      <tr key={s.id} className="group transition-colors hover:bg-slate-50/30">
                        <td
                          style={{ position: 'sticky', left: 0, width: 36, minWidth: 36 }}
                          className="bg-white px-0.5 py-1.5 text-[9px] md:text-[10.5px] font-black text-slate-400 text-center border-r border-b border-slate-100 z-20 transition-colors uppercase"
                        >
                        ...
                        {showNisKelas && (
                          <td
                            style={{ position: 'sticky', left: 36, width: 76, minWidth: 76 }}
                            className="bg-white px-1 py-1.5 text-[9.5px] md:text-[10.5px] font-black text-slate-500 text-center border-r border-b border-slate-100 z-20 transition-all duration-300 tracking-normal"
                          >
                        ...
                        <td
                          ...
                          className="bg-white px-2 py-1.5 text-[10.5px] md:text-[11px] font-bold text-slate-800 border-r border-b border-slate-100 z-20 transition-all duration-300 overflow-hidden shadow-[2px_0_5px_rgba(0,0,0,0.05)]"
                        >
                          <div className="h-10 flex items-center whitespace-nowrap overflow-hidden" title={target.name}>
                        ...
                        {!showNotes ? (
                          <>
                            <td className="px-1 py-1.5 border-r border-b border-slate-100 text-center bg-emerald-50/10">
                              <div className={`w-full text-center text-[11px] font-black h-10 flex items-center justify-center ${!target.hafalanJuz || target.hafalanJuz === "0" || target.hafalanJuz === "-" ? "text-emerald-200" : "text-emerald-700"}`}>
                                {target.hafalanJuz === "-" || !target.hafalanJuz ? "0" : target.hafalanJuz}
                              </div>
                            </td>
                            <td className="px-1 py-1.5 border-r border-b border-slate-100 text-center bg-emerald-50/10">
                              <div className={`w-full text-center text-[11px] font-black h-10 flex items-center justify-center ${!target.hafalanHal || target.hafalanHal === "0" || target.hafalanHal === "-" ? "text-emerald-200" : "text-emerald-700"}`}>
                            ...
                            <td className="px-1.5 py-1.5 border-r border-b border-slate-100 bg-emerald-50/5">
                              <div className="flex items-center justify-center gap-1">
                                <div className="w-21.25 px-1 text-[8.5px] font-black bg-white border border-slate-100 rounded-lg h-9 shadow-sm flex items-center justify-center uppercase text-slate-800 whitespace-nowrap overflow-hidden tracking-tighter">
                                  {target.manzilTarget || "-"}
                                </div>
                                {(() => {
                                  ...
                                  return (
                                    <div
                                      className={`flex items-center border rounded-lg px-2 h-9 shadow-sm ${isManzilTargetKurang ? "bg-red-50 border-red-200" : "bg-white border-slate-100"}`}
                                      title={isManzilTargetKurang ? `Target kurang dari standar ATM (${Number(target.manzilAtm) * activeDays.length} Hal/Pekan)` : ""}
                                    >
                            ...
                            <td className="px-1 py-1.5 border-r-2 border-b border-slate-100 bg-emerald-50/5 text-center">
                              <div
                                className={`w-9 h-9 mx-auto flex items-center justify-center text-[11px] font-black rounded-lg border border-slate-100 bg-white shadow-sm ${
                                  target.manzilKet === "A" ? "text-amber-500" : target.manzilKet === "C" ? "text-rose-500" : "text-emerald-600"
                                }`}
                              >
                                {target.manzilKet || "-"}
                              </div>
                            ...
                            <td className="px-1 py-1.5 border-r border-b border-slate-100 text-center bg-emerald-50/5 h-10">
                              <div className="flex items-center justify-center gap-1">
                                ...
                                {(() => {
                                  ...
                                  return (
                                    <div
                                      className={`flex items-center border rounded-lg px-2 h-9 shadow-sm ${isSabqiTargetKurang ? "bg-red-50 border-red-200" : "bg-white border-slate-100"}`}
                                      title={isSabqiTargetKurang ? `Target kurang dari standar ATM (${Number(target.sabqiAtm) * activeDays.length} Hal/Pekan)` : ""}
                                    >
                            ...
                            <td className="px-1 py-1.5 border-r-2 border-b border-slate-100 bg-emerald-50/5 text-center">
                              <div
                                className={`w-9 h-9 mx-auto flex items-center justify-center text-[11px] font-black rounded-lg border border-slate-100 bg-white shadow-sm ${
                                  target.sabqiKet === "A" ? "text-amber-500" : target.sabqiKet === "C" ? "text-rose-500" : "text-emerald-600"
                                }`}
                              >
                                {target.sabqiKet || "-"}
                              </div>
                            ...
                            <td className="px-1 py-1.5 border-b border-slate-100 bg-emerald-50/5 text-center">
                              <div
                                className={`w-9 h-9 mx-auto flex items-center justify-center text-[11px] font-black rounded-lg border border-slate-100 bg-white shadow-sm ${
                                  target.sabaqKet === "A" ? "text-amber-500" : target.sabaqKet === "C" ? "text-rose-500" : "text-emerald-600"
                                }`}
                              >
                                {target.sabaqKet || "-"}
                              </div>
                          ...
                          <td className="px-2 lg:px-4 py-1.5 border-b border-slate-100 bg-amber-50/5">
                            <div className="w-full h-auto min-h-10 px-3 py-2 text-[10.5px] font-bold text-slate-700 bg-white border border-amber-100 rounded-2xl shadow-sm flex flex-col justify-center transition-all">
                              {/* SUPERVISOR SECTION */}
                              <div className="flex flex-col md:flex-row md:items-start gap-1.5 md:gap-2 pt-1 flex-1">
                                <span className="text-[8px] md:mt-0.5 font-black text-amber-500 uppercase tracking-widest bg-amber-50 px-1.5 py-0.5 rounded border border-amber-100 shrink-0 w-fit md:w-18 inline-flex items-center justify-center">
                                  Supervisor
                                </span>
                                {isReadOnly ? (
                                  <textarea
                                    rows={1}
                                    defaultValue={target.supervisorNote}
                                    onFocus={(e) => {
                                    ...
                                    onBlur={(e) => {
                                      ...
                                      if (e.target.value !== target.supervisorNote) {
                                        handleSaveSupervisorNote(target, e.target.value);
                                      }
                                    }}
                                    style={{ maxHeight: "90px" }}
                                    placeholder="BERIKAN CATATAN UNTUK PENGAMPU"
                                    className="flex-none md:flex-1 w-full px-2 py-1.5 md:py-1 text-[10px] font-bold text-amber-800 bg-amber-50/30 hover:bg-amber-50 border border-transparent hover:border-amber-200 focus:bg-white focus:border-amber-400 focus:ring-2 focus:ring-amber-100 rounded-md outline-none transition-all resize-none h-8 min-h-8 md:h-6 md:min-h-6 md:-mt-1.5 overflow-hidden focus:overflow-y-auto placeholder:text-slate-400 placeholder:font-normal placeholder:italic placeholder:tracking-tighter placeholder:text-[7.5px] md:placeholder:text-[8px]"
                                  />
                                ) : (
                                  <span className="whitespace-normal leading-snug text-[10px] font-bold text-amber-800 flex-1 mt-0.5 md:mt-0 px-2">
                                    {target.supervisorNote || <span className="text-slate-400 font-normal italic uppercase tracking-tighter text-[7px] md:text-[8px]">Belum ada catatan...</span>}
                                  </span>
                                )}
                              </div>

                              {/* USTADZ SECTION */}
                              <div className="flex flex-col md:flex-row md:items-start gap-1.5 md:gap-2 pt-2 mt-2 border-t border-dashed border-amber-100">
                                <span className="text-[8px] md:mt-0.5 font-black text-slate-400 uppercase tracking-widest bg-slate-50 px-1.5 py-0.5 rounded border border-slate-100 shrink-0 w-fit md:w-18 inline-flex items-center justify-center">
                                  Ustadz
                                </span>
                                <span className="whitespace-normal leading-snug text-[10px] font-bold flex-1 mt-0.5 md:mt-0 px-2">
                                  {target.teacherNote || <span className="text-slate-400 font-normal italic uppercase tracking-tighter text-[7px] md:text-[8px]">Belum ada catatan...</span>}
                                </span>
                              </div>
                            </div>
                          </td>
          ...
          <div className="bg-slate-50 border-t border-slate-100 px-3 md:px-6 py-3 flex flex-row justify-between items-center gap-2 rounded-b-xl">
            <div className="flex items-center gap-2 md:gap-4">
              ...
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-tight whitespace-nowrap">
                ...
                <span className="text-primary-600 ml-0.5">{filteredStudents.length}</span>
              </p>
            </div>
