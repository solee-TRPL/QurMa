...
export const Settings: React.FC<SettingsProps> = ({ user, tenant, onProfileUpdate, onTenantUpdate }) => {
  ...
  const [showRegencyPicker, setShowRegencyPicker] = useState(false);
  ...
  const onProvinceChange = async (provId: string, provName: string, isAutoInit = false) => {
    setProfileForm((prev) => ({ ...prev, provinceId: provId, province: provName, ...(isAutoInit ? {} : { regencyId: "", districtId: "", villageId: "", city: "", district: "", village: "" }) }));
    try {
  ...
  const onRegencyChange = async (regId: string, regName: string, isAutoInit = false) => {
    const cleanedCity = regName.replace(/^(KABUPATEN|KOTA|KAB\.|KAB)\s+/i, "").trim();
    setProfileForm((prev) => ({ ...prev, regencyId: regId, city: cleanedCity, ...(isAutoInit ? {} : { districtId: "", villageId: "", district: "", village: "" }) }));
    try {
  ...
  const onDistrictChange = async (distId: string, distName: string, isAutoInit = false) => {
    setProfileForm((prev) => ({ ...prev, districtId: distId, district: distName, ...(isAutoInit ? {} : { villageId: "", village: "" }) }));
    try {
  ...
  useEffect(() => {
    if (regions.provinces.length > 0 && profileForm.province && !profileForm.provinceId) {
      ...
      if (found) {
        onProvinceChange(found.id, found.name, true);
      }
    }
  ...
  useEffect(() => {
    if (regions.districts.length > 0 && profileForm.district && !profileForm.districtId) {
      ...
      if (found) {
        onDistrictChange(found.id, found.name, true);
      }
    }
  ...
  useEffect(() => {
    if (tenant) {
      ...
      if (tenant.cycle_config) {
        let loadedPeriods = tenant.cycle_config.activePeriods && tenant.cycle_config.activePeriods.length > 0 ? tenant.cycle_config.activePeriods : [];

        // Hapus otomatis periode yang sudah lewat (expired)
        const today = new Date();
        today.setHours(0, 0, 0, 0);

        loadedPeriods = loadedPeriods.filter((p: any) => {
        ...
        // Jika semua habis, sediakan satu yang kosong
        if (loadedPeriods.length === 0) {
          loadedPeriods = [{ startDate: "", endDate: "" }];
        }
  ...
  const toggleDay = (day: number) => {
    setSystemForm((prev) => {
      const activeDays = prev.activeDays.includes(day) ? prev.activeDays.filter((d) => d !== day) : [...prev.activeDays, day].sort();
            ...
            <div className="max-w-2xl flex flex-row items-center gap-4 lg:gap-6 shrink-0">
              <div className="relative group shrink-0">
                <div className="w-14 h-14 lg:w-20 lg:h-20 bg-white rounded-xl lg:rounded-xl flex items-center justify-center text-slate-300 text-xl lg:text-2xl font-black border-2 border-slate-300 overflow-hidden group-hover:scale-95 transition-transform duration-500">
                  {user.avatar_url ? <img src={user.avatar_url} alt={user.full_name} className="w-full h-full object-cover" /> : <span>{user.full_name.charAt(0)}</span>}
                </div>
                <button type="button" onClick={() => fileInputRef.current?.click()} className="absolute -bottom-1 -right-1 p-1.5 lg:p-2 bg-jade-600 text-white rounded-xl shadow-none hover:bg-jade-700 transition-all active:scale-90">
              ...
              <div className="text-left py-1 flex-1">
                ...
                <p className="text-[9px] lg:text-[11px] font-black text-jade-600 mt-1 uppercase tracking-widest leading-none px-2 py-1 lg:py-1.5 bg-jade-50 border border-jade-100/50 rounded-lg w-fit">
                  {user.role === UserRole.ADMIN
                    ? "Admin"
            ...
            <div className="max-w-2xl grid grid-cols-1 md:grid-cols-2 gap-x-2 gap-y-4 lg:gap-y-4 pt-3 lg:pt-4 border-t border-slate-100 overflow-y-auto lg:overflow-visible pr-1 custom-scrollbar">
              ...
              {user.role === UserRole.TEACHER && (
                <>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[9px] lg:text-[10.5px] font-black text-slate-400 uppercase tracking-widest ml-1">
                      NIK <span className="normal-case font-medium text-slate-300">(Nomor Induk Kependudukan)</span>
                    </label>
                    <input
                      type="text"
                      maxLength={16}
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[9px] lg:text-[10.5px] font-black text-slate-400 uppercase tracking-widest ml-1">
                      NIP <span className="normal-case font-medium text-slate-300">(Nomor Induk Pegawai)</span>
                    </label>
                    <div className="relative">
                      <input
                        type="text"
              ...
              {user.role === UserRole.SANTRI && (
                <>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <div
                        className="w-full px-4 lg:px-5 py-2 lg:py-2.5 bg-white border-2 border-slate-300 cursor-pointer hover:border-slate-400 rounded-xl transition-all h-8.5 lg:h-9.5 flex items-center justify-between"
                        onClick={() => setShowProvincePicker(!showProvincePicker)}
                      >
                        <span className="text-[10px] lg:text-[11px] font-black tracking-widest text-slate-700 pointer-events-none truncate uppercase pr-4">
                          {profileForm.province || "PILIH PROVINSI"}
                        </span>
                        <ChevronDown className={`absolute right-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 pointer-events-none transition-transform ${showProvincePicker ? "rotate-180" : ""}`} />
                      </div>
                      ...
                      {showProvincePicker && (
                        <>
                          ...
                          />
                          <div className="absolute top-full left-0 right-0 mt-1 bg-white border-2 border-slate-300 rounded-xl shadow-none z-50 py-1 max-h-48 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent animate-in fade-in zoom-in-95 duration-200">
                            {regions.provinces.map((prov) => (
                              <div
                                key={prov.id}
                                className={`px-4 py-2.5 border-b border-slate-100 last:border-0 text-[10px] font-black tracking-widest uppercase cursor-pointer hover:bg-slate-50 transition-colors ${profileForm.provinceId === prov.id ? "text-jade-600 bg-jade-50" : "text-slate-600"}`}
                                onClick={(e) => {
                                  e.stopPropagation();
                                  onProvinceChange(prov.id, prov.name);
                                  setShowProvincePicker(false);
                                }}
                              >
                                {prov.name}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <div
                        className={`w-full px-4 lg:px-5 py-2 lg:py-2.5 rounded-xl transition-all h-8.5 lg:h-9.5 flex items-center justify-between border-2 ${!profileForm.provinceId ? "bg-slate-50 border-slate-200 cursor-not-allowed opacity-50" : "bg-white border-slate-300 cursor-pointer hover:border-slate-400"}`}
                        onClick={() => profileForm.provinceId && setShowRegencyPicker(!showRegencyPicker)}
                      >
                        <span className="text-[10px] lg:text-[11px] font-black tracking-widest text-slate-700 pointer-events-none truncate uppercase pr-4">
                          {profileForm.city || "PILIH KOTA/KABUPATEN"}
                        </span>
                        <ChevronDown className={`absolute right-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 pointer-events-none transition-transform ${showRegencyPicker ? "rotate-180" : ""}`} />
                      </div>

                      {showRegencyPicker && profileForm.provinceId && (
                        <>
                          <div
                            className="fixed inset-0 z-40"
                            onClick={(e) => {
                              e.stopPropagation();
                              setShowRegencyPicker(false);
                            }}
                          />
                          <div className="absolute top-full left-0 right-0 mt-1 bg-white border-2 border-slate-300 rounded-xl shadow-none z-50 py-1 max-h-48 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent animate-in fade-in zoom-in-95 duration-200">
                            {regions.regencies.map((r) => {
                              const rName = r.name.replace(/^(KABUPATEN|KOTA|KAB\.|KAB)\s+/i, "").trim();
                              return (
                                <div
                                  key={r.id}
                                  className={`px-4 py-2.5 border-b border-slate-100 last:border-0 text-[10px] font-black tracking-widest uppercase cursor-pointer hover:bg-slate-50 transition-colors ${profileForm.regencyId === r.id ? "text-jade-600 bg-jade-50" : "text-slate-600"}`}
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    onRegencyChange(r.id, r.name);
                                    setShowRegencyPicker(false);
                                  }}
                                >
                                  {rName}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <div
                        className={`w-full px-4 lg:px-5 py-2 lg:py-2.5 rounded-xl transition-all h-8.5 lg:h-9.5 flex items-center justify-between border-2 ${!profileForm.regencyId ? "bg-slate-50 border-slate-200 cursor-not-allowed opacity-50" : "bg-white border-slate-300 cursor-pointer hover:border-slate-400"}`}
                        onClick={() => profileForm.regencyId && setShowDistrictPicker(!showDistrictPicker)}
                      >
                        <span className="text-[10px] lg:text-[11px] font-black tracking-widest text-slate-700 pointer-events-none truncate uppercase pr-4">
                          {profileForm.district || "PILIH KECAMATAN"}
                        </span>
                        <ChevronDown className={`absolute right-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 pointer-events-none transition-transform ${showDistrictPicker ? "rotate-180" : ""}`} />
                      </div>

                      {showDistrictPicker && profileForm.regencyId && (
                        <>
                          <div
                            className="fixed inset-0 z-40"
                            onClick={(e) => {
                          ...
                          <div className="absolute top-full left-0 right-0 mt-1 bg-white border-2 border-slate-300 rounded-xl shadow-none z-50 py-1 max-h-48 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent animate-in fade-in zoom-in-95 duration-200">
                            {regions.districts.map((d) => (
                              <div
                                key={d.id}
                                className={`px-4 py-2.5 border-b border-slate-100 last:border-0 text-[10px] font-black tracking-widest uppercase cursor-pointer hover:bg-slate-50 transition-colors ${profileForm.districtId === d.id ? "text-jade-600 bg-jade-50" : "text-slate-600"}`}
                                onClick={(e) => {
                                  e.stopPropagation();
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <div
                        className={`w-full px-4 lg:px-5 py-2 lg:py-2.5 rounded-xl transition-all h-8.5 lg:h-9.5 flex items-center justify-between border-2 ${!profileForm.districtId ? "bg-slate-50 border-slate-200 cursor-not-allowed opacity-50" : "bg-white border-slate-300 cursor-pointer hover:border-slate-400"}`}
                        onClick={() => profileForm.districtId && setShowVillagePicker(!showVillagePicker)}
                      >
                        <span className="text-[10px] lg:text-[11px] font-black tracking-widest text-slate-700 pointer-events-none truncate uppercase pr-4">
                          {profileForm.village || "PILIH KELURAHAN/DESA"}
                        </span>
                        <ChevronDown className={`absolute right-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 pointer-events-none transition-transform ${showVillagePicker ? "rotate-180" : ""}`} />
                      </div>

                      {showVillagePicker && profileForm.districtId && (
                        <>
                          <div
                            className="fixed inset-0 z-40"
                            onClick={(e) => {
                          ...
                          />
                          <div className="absolute bottom-full mb-1 md:bottom-auto md:top-full md:mt-1 left-0 right-0 bg-white border-2 border-slate-300 rounded-xl shadow-none z-50 py-1 max-h-48 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent animate-in fade-in zoom-in-95 duration-200">
                            {regions.villages.map((v) => (
                              <div
                                key={v.id}
                                className={`px-4 py-2.5 border-b border-slate-100 last:border-0 text-[10px] font-black tracking-widest uppercase cursor-pointer hover:bg-slate-50 transition-colors ${profileForm.villageId === v.id ? "text-jade-600 bg-jade-50" : "text-slate-600"}`}
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setProfileForm({ ...profileForm, villageId: v.id, village: v.name });
                                  setShowVillagePicker(false);
                                }}
                              >
                                {v.name}
              ...
              <div className="flex flex-row items-center gap-4 lg:gap-6 shrink-0">
                <div className="relative group shrink-0">
                  <div className="w-16 h-16 lg:w-24 lg:h-24 bg-white rounded-xl flex items-center justify-center text-slate-300 text-xl lg:text-3xl font-black border-2 border-slate-300 overflow-hidden group-hover:scale-95 transition-transform duration-500">
                    {tenant?.logo_url ? <img src={tenant.logo_url} alt="Logo" className="w-full h-full object-contain p-2" /> : <Building className="w-8 lg:w-12 h-8 lg:h-12 text-slate-200" />}
                  </div>
                  {!isReadOnly && (
                    <button
                      type="button"
              ...
              <div className="space-y-6">
                <div className="flex items-end gap-2 lg:gap-3">
                  <div className="flex-1 space-y-1.5">
                    ...
                    <div
                      className={`relative w-full px-4 lg:px-5 py-2 lg:py-2.5 ${isReadOnly ? "bg-slate-50 border-2 border-slate-200 text-slate-400 cursor-not-allowed" : "bg-white border-2 border-slate-300 cursor-pointer hover:border-slate-400"} rounded-xl transition-all h-8.5 lg:h-9.5 flex items-center justify-between`}
                      onClick={() => !isReadOnly && setShowStartMonthPicker(!showStartMonthPicker)}
                    >
                      <span className="text-[10px] lg:text-[11px] font-black uppercase tracking-widest text-slate-700 pointer-events-none">{MONTH_NAMES[systemForm.academicYearStartMonth]}</span>
                      <ChevronDown className={`w-3.5 h-3.5 text-slate-400 pointer-events-none transition-transform ${showStartMonthPicker ? "rotate-180" : ""}`} />

                      {showStartMonthPicker && !isReadOnly && (
                        <>
                          <div
                            className="fixed inset-0 z-40"
                            onClick={(e) => {
                          ...
                          <div className="absolute top-full left-0 right-0 mt-1 bg-white border-2 border-slate-300 rounded-xl shadow-none z-50 py-1 max-h-48 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent animate-in fade-in zoom-in-95 duration-200">
                            {MONTH_NAMES.map((m, idx) => (
                              <div
                                key={idx}
                                className={`px-3 py-2.5 border-b border-slate-100 last:border-0 text-[10px] font-black tracking-widest cursor-pointer text-center hover:bg-slate-50 transition-colors ${idx === systemForm.academicYearStartMonth ? "text-jade-600 bg-jade-50" : "text-slate-600"}`}
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setSystemForm({ ...systemForm, academicYearStartMonth: idx });
                                  setShowStartMonthPicker(false);
                                }}
                              >
                                {m}
                ...
                <div className="space-y-2 border-t border-slate-100 pt-2">
                  <div className="flex items-center justify-between pb-2">
                    <label className="text-[9px] lg:text-[10.5px] font-black text-slate-400 uppercase tracking-widest ml-1">Priode Aktif Tahun Ajaran</label>
                  </div>

                  {systemForm.activePeriods.length === 0 ? (
                    <p className="text-[10px] lg:text-[11px] font-medium text-slate-400 italic px-2 py-4 text-center bg-slate-50 rounded-xl border border-slate-100">Belum ada masa aktif yang diatur. Tahun ajaran akan dianggap non-aktif.</p>
                  ) : (
                          ...
                          <div className="flex-1 space-y-1.5">
                            ...
                            <CustomDatePicker
                              ...
                              onChange={(val) => {
                                ...
                                // Validasi terhadap periode sebelumnya
                                if (idx > 0 && val) {
                                  const prevEndDateStr = newPeriods[idx - 1].endDate;
                                  if (prevEndDateStr) {
                                    const prevEnd = new Date(prevEndDateStr);
                                    const currentStart = new Date(val);
                                    if (currentStart <= prevEnd) {
                                      addNotification({
                                        type: "error",
                                        title: "Tanggal Tidak Valid",
                                        message: `Mulai Periode ${idx + 1} harus setelah Selesai Periode ${idx}.`,
                                      });
                                      return;
                                    }
                                  }
                                ...
                                let newEndDate = newPeriods[idx].endDate;
                                if (val) {
                                  const start = new Date(val);
                                  let shouldAdjust = !newEndDate;

                                  if (newEndDate) {
                                    const end = new Date(newEndDate);
                                    if (start >= end) shouldAdjust = true;
                                  }
                                ...
                                newPeriods[idx] = { ...newPeriods[idx], startDate: val, endDate: newEndDate };
                                setSystemForm({ ...systemForm, activePeriods: newPeriods });
                              }}
                              className={`w-full ${isReadOnly ? "bg-slate-100 border-2 border-slate-200 text-slate-400" : "bg-white border-2 border-slate-300"} rounded-xl transition-all h-8.5 lg:h-9.5`}
                            />
                          </div>
                          <div className="flex-1 space-y-1.5">
                            ...
                            <CustomDatePicker
                              ...
                              onChange={(val) => {
                                const newPeriods = [...systemForm.activePeriods];
                                const startDateStr = newPeriods[idx].startDate;
                                ...
                                if (val && startDateStr) {
                                  ...
                                  if (end <= start) {
                                    addNotification({
                                      type: "error",
                                      title: "Tanggal Tidak Valid",
                                      message: "Tanggal Selesai Periode harus lebih besar (setelah) tanggal Mulai Periode.",
                                    });
                                    return;
                                  }
                                }

                                // Validasi terhadap periode setelahnya
                                if (idx < newPeriods.length - 1 && val) {
                                  const nextStartDateStr = newPeriods[idx + 1].startDate;
                                  if (nextStartDateStr) {
                                    const nextStart = new Date(nextStartDateStr);
                                    const currentEnd = new Date(val);
                                    if (currentEnd >= nextStart) {
                                      addNotification({
                                        type: "error",
                                        title: "Tanggal Tidak Valid",
                                        message: `Selesai Periode ${idx + 1} tidak boleh melebihi atau sama dengan Mulai Periode ${idx + 2}.`,
                                      });
                                      return;
                                    }
                                  }
                                ...
                                newPeriods[idx] = { ...newPeriods[idx], endDate: val };
                                setSystemForm({ ...systemForm, activePeriods: newPeriods });
                              }}
                              className={`w-full ${isReadOnly ? "bg-slate-100 border-2 border-slate-200 text-slate-400" : "bg-white border-2 border-slate-300"} rounded-xl transition-all h-8.5 lg:h-9.5`}
                            />
                  ...
                  {!isReadOnly && systemForm.activePeriods.length < 5 && (
                      ...
                      <button
                        ...
                        onClick={() =>
                          setSystemForm((prev) => {
                            ...
                            if (prev.activePeriods.length > 0) {
                              const lastPeriod = prev.activePeriods[prev.activePeriods.length - 1];
                              if (lastPeriod.endDate) {
                                const end = new Date(lastPeriod.endDate);

                                const nextStart = new Date(end);
                                nextStart.setDate(nextStart.getDate() + 1);
                                newStartDate = `${nextStart.getFullYear()}-${String(nextStart.getMonth() + 1).padStart(2, "0")}-${String(nextStart.getDate()).padStart(2, "0")}`;

                                const nextEnd = new Date(nextStart);
                                nextEnd.setDate(nextEnd.getDate() + 1);
                                newEndDate = `${nextEnd.getFullYear()}-${String(nextEnd.getMonth() + 1).padStart(2, "0")}-${String(nextEnd.getDate()).padStart(2, "0")}`;
                              }
                            }

                            return {
                              ...prev,
                              activePeriods: [...prev.activePeriods, { startDate: newStartDate, endDate: newEndDate }],
                            };
                          })
                        }
                    ...
                    <table className="min-w-full border-separate border-spacing-0">
                      <thead>
                        <tr className="bg-slate-300">
                          <th className="px-4 py-2.5 text-center md:text-left text-[9px] font-black text-slate-800 uppercase tracking-[0.2em] border-b border-r border-slate-200">Semester 1</th>
                          <th className="px-4 py-2.5 text-center md:text-left text-[9px] font-black text-slate-800 uppercase tracking-[0.2em] border-b border-slate-200">Semester 2</th>
                        </tr>
                      </thead>
                      <tbody>
                        <tr>
                          <td className="px-4 py-3.5 text-center md:text-left text-[11px] font-black text-jade-700 bg-white border-r border-slate-100 uppercase tracking-tight">{getSemesterInfo(systemForm.academicYearStartMonth).s1}</td>
                          <td className="px-4 py-3.5 text-center md:text-left text-[11px] font-black text-emerald-700 bg-white uppercase tracking-tight">{getSemesterInfo(systemForm.academicYearStartMonth).s2}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
                  ...
                  return (
                    <button
                      ...
                      className={`flex-1 px-1 lg:px-2 py-2 lg:py-2.5 rounded-xl text-[8.5px] lg:text-[9.5px] font-black uppercase tracking-widest border-2 transition-all h-9 lg:h-auto flex items-center justify-center ${
                        isActive ? "bg-jade-600 border-jade-600 text-white" : "bg-white border-jade-500 text-jade-600 hover:bg-jade-50"
                      }`}
                    >
                      <span className="md:hidden">{day.label.substring(0, 3)}</span>
                      <span className="hidden md:block">{day.label}</span>
                    </button>
                  );
                })}
              </div>
              <p className="text-[6.5px] md:text-[9px] font-bold text-slate-500 uppercase tracking-tight md:tracking-tight italic opacity-60 w-full text-center md:text-left">
