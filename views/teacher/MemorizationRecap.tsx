...
export const MemorizationRecap: React.FC<MemorizationRecapProps> = ({ user }) => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      setLoading(true);
      try {
        const [allStudents, tenantData, allHalaqahs] = await Promise.all([getStudents(user.tenant_id), getTenant(user.tenant_id), getHalaqahs(user.tenant_id)]);
  ...
  useEffect(() => {
    const fetchReport = async () => {
      ...
      setLoading(true);
      try {
        const [weekData, targets] = await Promise.all([getWeeklyMemorization(selectedStudent.id, selectedWeek), getWeeklyTargets([selectedStudent.id], selectedWeek)]);
  ...
  return (
    <div className="flex flex-col lg:flex-row h-[calc(100vh-140px)] gap-4 lg:gap-6 animate-fade-in overflow-hidden">
      {/* 1. SIDEBAR SANTRI */}
      <div
        className={`fixed inset-0 z-50 lg:relative lg:inset-auto lg:flex w-full lg:w-64 bg-slate-900/40 lg:bg-transparent backdrop-blur-sm lg:backdrop-blur-none transition-all duration-300 ${isSidebarOpen ? "opacity-100 visible" : "opacity-0 invisible lg:opacity-100 lg:visible"}`}
        onClick={() => setIsSidebarOpen(false)}
      >
        <div
          className={`w-72 lg:w-full h-full bg-white lg:rounded-xl border-2 border-slate-300 flex flex-col overflow-hidden shadow-2xl lg:shadow-none transition-transform duration-300 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
                ...
                filteredStudents.map((student) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 p-3 rounded-xl transition-all duration-200 group ${
                      selectedStudent?.id === student.id ? "bg-jade-600 text-white shadow-lg shadow-jade-100 border-2 border-jade-600" : "bg-white text-slate-600 border-2 border-transparent hover:bg-slate-50 hover:border-slate-100"
                    }`}
                  >
                    <div
                      className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 border-2 transition-colors ${
                    ...
                    <div className="text-left min-w-0">
                      <p className={`text-[11px] font-black truncate uppercase tracking-tight ${selectedStudent?.id === student.id ? "text-white" : "text-slate-700"}`}>{student.full_name}</p>
                      <p className={`text-[8.5px] font-black uppercase tracking-widest leading-none mt-1 ${selectedStudent?.id === student.id ? "text-jade-100" : "text-slate-400"}`}>NIS: {student.nis || "-"}</p>
                    </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="bg-white rounded-xl border-2 border-slate-300 p-3 lg:p-4 mb-4 flex flex-col lg:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3 w-full lg:w-auto">
            ...
            <div>
              <h2 className="text-sm font-black text-slate-800 uppercase tracking-tight leading-none mb-1">{selectedStudent?.full_name || "Rekap Hafalan"}</h2>
          ...
          <div className="flex items-center gap-2 w-full lg:w-auto overflow-x-auto no-scrollbar py-1 lg:py-0">
            <div className="flex bg-white p-1 rounded-xl border-2 border-slate-300 h-10 items-center justify-center shrink-0">
              ...
              <div className="px-3 text-[10px] font-black text-jade-700 uppercase tracking-widest whitespace-nowrap min-30 text-center">PEKAN INI</div>
              <button
                onClick={() => {
                  const d = new Date(selectedWeek);
                  d.setDate(d.getDate() + 7);
        ...
        <div className="flex-1 bg-white rounded-xl border-2 border-slate-300 flex flex-col overflow-hidden relative">
          {!selectedStudent ? (
            <div className="flex-1 flex flex-col items-center justify-center">
              <EmptyState icon="user" message="Pilih Santri" description="Silakan pilih santri dari daftar untuk melihat rekap hafalan." />
            </div>
          ) : (
            <>
              {/* Stats Banner */}
                              ...
                              return (
                                <tr key={`${date}-${type}`} className={`${isToday ? "bg-emerald-50/10" : "hover:bg-slate-50/20"} transition-colors group`}>
                                  ...
                                  <td className={`px-2 py-4 sticky left-24 z-30 w-24 border-b border-r border-slate-200 text-center transition-colors ${isToday ? "bg-emerald-50" : "bg-white"}`}>
                                    <span className={`text-[10px] font-black uppercase tracking-widest ${isToday ? "text-emerald-700" : "text-slate-400"}`}>{getTypeLabel(type)}</span>
                                  </td>
                                  <td className={`px-6 py-4 border-b border-r border-slate-200 ${isToday ? "bg-emerald-50/20" : ""}`}>
                                    <div className="flex items-center gap-3">
                                      <div className="flex flex-col">
                                        <p className={`text-[11px] font-black uppercase tracking-widest ${rec?.surah_name ? "text-slate-800" : "text-slate-200"}`}>{rec?.surah_name || "- SURAT -"}</p>
                                  ...
                                  <td className={`px-6 py-4 border-b-2 border-slate-200 text-center ${isToday ? "bg-emerald-50/20" : ""}`}>
                                    {rec && rec.status && rec.status !== MemorizationStatus.EMPTY ? (
                                      <span
                                        className={`inline-flex px-4 py-1.5 rounded-xl border-2 text-[9px] font-black uppercase tracking-[0.15em] ${
                                      ...
                                      <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest italic">- KOSONG -</span>
                                    )}
                                  </td>
                                </tr>
                              );
