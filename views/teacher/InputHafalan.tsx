...
export const InputHafalan: React.FC<InputHafalanProps> = ({ user, tenantId, onSetUnsavedChanges, saveTrigger, onSaveSuccess, isGlobalModalOpen }) => {
  ...
  const fetchData = async (force: boolean = false) => {
    ...
    setLoading(true);
    try {
      const [allHalaqahs, tenantData] = await Promise.all([getHalaqahs(user.tenant_id), getTenant(user.tenant_id)]);
  ...
  return (
    <div className="flex flex-col lg:flex-row h-[calc(100vh-140px)] gap-4 lg:gap-6 animate-fade-in overflow-hidden no-scrollbar">
                ...
                <div className="flex items-center justify-between lg:justify-end gap-1.5 w-full lg:w-auto">
                  ...
                  <div className="flex items-center gap-1 sm:gap-1.5 shrink-0">
                    ...
                    <button
                      ...
                      disabled={isSaving}
                      className="flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-3 lg:px-5 py-1.5 lg:py-2.5 bg-primary-600 hover:bg-primary-700 text-white rounded-xl shadow-none transition-all active:scale-95 group border-2 border-primary-600 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {isSaving ? <div className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <Save className="w-3.5 h-3.5 group-hover:rotate-12 transition-transform" />}
                      <span className="text-[8px] sm:text-[9px] font-black uppercase tracking-widest">{isSaving ? "Menyimpan" : "Simpan"}</span>
                    </button>
                  </div>
                </div>
              </div>
                                      ...
                                      <tr key={`${date}-${type}`} className={`group ${isToday ? "bg-emerald-50/40" : "hover:bg-emerald-50/40"}`}>
                                        ...
                                        <td className={`px-2 py-1 border-b border-slate-200 ${openSurahDropdown === `desktop-${date}-${type}` ? "relative z-60" : ""}`}>
                                          <div className="flex items-center gap-1 justify-center min-w-36 lg:min-w-50">
                                            ...
                                            {/* AYAT - NOW "ayat_end" */}
                                            <div
                                              className={`flex items-center h-8 px-1 flex-none w-17 border rounded-lg transition-all shadow-sm ${
                                            ...
                                            >
                                              {(() => {
                                                ...
                                                return (
                                                  <input
                                                    type="number"
                                                    disabled={!isSurahSelected || isHafalanAwal}
                                                    placeholder="0"
                                                    min={0}
      ...
      {/* Input Modal */}
      {isAddModalOpen && (
        <div className="fixed inset-0 z-9999 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md animate-in fade-in duration-300 lg:pl-64 pt-20" onClick={() => setIsAddModalOpen(false)}>
      ...
      {isProgressModalOpen && (
        <div className="fixed inset-0 z-999999 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xl animate-fade-in lg:pl-64 pt-16" onClick={() => setIsProgressModalOpen(false)}>
