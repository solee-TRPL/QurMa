...
export const GuardianDirectory: React.FC<{ tenantId: string; user: UserProfile }> = ({ tenantId, user }) => {
  ...
  const filteredGuardians = useMemo(() => {
    ...
    return guardianData.filter(({ guardian, students }) => guardian.full_name.toLowerCase().includes(term) || students.some((s) => s.full_name.toLowerCase().includes(term)));
  }, [guardianData, search]);
            ...
            {filteredGuardians.map(({ guardian, students }) => (
              <div key={guardian.id} className="p-5 flex flex-col md:flex-row items-start justify-between gap-6 group">
                ...
                <div className="flex items-center gap-4 w-full md:w-1/3 min-70">
                  <div className="w-12 h-12 bg-slate-100 rounded-full flex items-center justify-center text-slate-500 font-bold text-lg border border-slate-200 shrink-0 capitalize">{guardian.full_name.charAt(0)}</div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1 text-sm text-slate-500">
                      {guardian.whatsapp_number ? (
                        <a href={formatWhatsAppUrl(guardian.whatsapp_number)} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 hover:text-green-600 transition-colors">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-500">
                              NIS: {student.nis || "-"} • Juz {student.current_juz}
                            </p>
                          </div>
                        </div>
                      </div>
