...
import { getStudents, getStudentsByHalaqah, createStudent, getHalaqahs, createUser, createStudentNote, updateStudent, deleteStudent, updateUser, getUsers, getAchievements, createAchievement, deleteAchievement, updateAchievement, getStudentNotes, deleteStudentNote, updateStudentNote, getClasses, markNoteAsSeen } from '../../services/dataService';
...
import { Search, Plus, X, Save, MessageSquare, ChevronRight, User, Trash2, Edit3, Phone, Trophy, Calendar, Check, Trash, FileText, Quote, ChevronDown, Filter, AlertTriangle, RotateCcw, Eye, EyeOff, Reply, CheckCircle2, Info } from 'lucide-react';
            ...
            <div className="bg-white rounded-xl w-full max-w-sm overflow-hidden flex flex-col animate-in zoom-in-95 duration-200 border-2 border-slate-200" onClick={e => e.stopPropagation()}>
                <div className="px-5 py-3 border-b border-slate-100 flex justify-between items-center bg-white rounded-t-xl">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button 
                            onClick={onClose} 
                            className="p-2 hover:bg-slate-50 rounded-lg text-slate-400 hover:text-slate-600 transition-all active:scale-90 shrink-0"
                        >
                            <X className="w-4 h-4" />
                        </button>
                    </div>
                                        ...
                                        <div className={`grid transition-all duration-500 ease-in-out ${viewingReply === note.id ? 'grid-rows-[1fr] opacity-100 mt-2' : 'grid-rows-[0fr] opacity-0'}`}>
                                            <div className="overflow-hidden">
                                                <div className="p-2.5 bg-jade-50/50 border border-jade-100 rounded-xl">
                                                    ...
                                                    <p className="text-[9px] font-bold text-jade-700 leading-tight uppercase tracking-tight">
                                                        {note.reply_content}
                                                    </p>
                                                    {note.replied_at && (
                ...
                <form onSubmit={handleAdd} className="p-3 border-t border-slate-100 bg-slate-50/30 space-y-2.5">
                    <div className="flex items-center gap-3">
                        ...
                        <input 
                            ...
                            className="flex-1 px-3.5 py-1.5 border border-slate-200 bg-white rounded-xl focus:ring-2 focus:ring-jade-100 focus:border-jade-400 shadow-sm transition-all text-[9px] uppercase font-bold text-slate-800 outline-none h-9 placeholder:text-slate-300 placeholder:font-black placeholder:uppercase placeholder:text-[8px]" 
                        />
                    </div>
                             ...
                             {mode === 'santri' && (
                                 <>
                                     <td className="w-37.5 min-37.5 max-37.5 px-4 py-3 text-center bg-slate-50/5 border-b border-slate-200 group-hover:bg-emerald-50 leading-tight">
                                         <div className="text-[11px] font-bold text-slate-700 whitespace-normal">
                                             {student.father_name || '-'}
                                         </div>
                                     ...
                                     <td 
                                         ...
                                         title={[student.address, student.village, student.district, student.city].filter(Boolean).join(', ') || '-'}
                                     >
                                         <span className="text-[11px] font-bold text-slate-700">
                                             {[student.address, student.village, student.district, student.city].filter(Boolean).join(', ') || '-'}
                                         </span>
                                     </td>
                                 </>
                             )}
