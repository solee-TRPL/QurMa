...
import { GraduationCap, Users, Search, Save, TrendingUp, Activity, ChevronRight, Search as SearchIcon, CheckCircle2, AlertCircle } from "lucide-react";
...
export const ManageStudentProgress: React.FC<{ user: UserProfile }> = ({ user }) => {
  ...
  useEffect(() => {
    ...
    const fetchProgress = async () => {
      try {
        const { data, error } = await supabase.from("student_semester_progress").select("progress_data").eq("student_id", selectedStudent.id).maybeSingle();
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const { error } = await supabase.from("student_semester_progress").upsert(
        {
  ...
  const filteredStudents = students.filter((s) => s.full_name.toLowerCase().includes(searchQuery.toLowerCase()) || (s.nis && s.nis.includes(searchQuery)));
  ...
  return (
    <div className="flex flex-col gap-5 animate-fade-in pb-10">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-center gap-4 bg-slate-100/30 p-2 rounded-3xl border border-slate-200/50 backdrop-blur-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1 flex flex-col gap-4">
          <div className="bg-white rounded-3xl border border-slate-200/60 shadow-sm overflow-hidden flex flex-col max-175">
              ...
              {filteredStudents.map((s) => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 p-3.5 rounded-2xl transition-all text-left group ${
                    selectedStudent?.id === s.id ? "bg-jade-50 border-jade-100 text-jade-700 shadow-sm ring-1 ring-jade-100/50" : "hover:bg-slate-50 text-slate-600 border border-transparent hover:border-slate-100"
                  }`}
                >
                  <div className="flex-1 overflow-hidden">
                    ...
                    <p className={`text-[9px] font-bold mt-0.5 ${selectedStudent?.id === s.id ? "text-jade-400" : "text-slate-400"}`}>NIS: {s.nis || "-"}</p>
                  </div>
        ...
        <div className="lg:col-span-3 flex flex-col gap-6">
          {selectedStudent ? (
            <>
              ...
              <div className="bg-white rounded-3xl p-5 border border-slate-200/60 shadow-sm flex flex-col md:flex-row justify-between items-center gap-6 relative overflow-hidden group">
                ...
                <div className="flex gap-3 w-full md:w-auto relative z-10">
                  <div className="flex-1 md:flex-none bg-slate-50 px-5 py-2.5 rounded-2xl border border-slate-100 text-center min-30">
                    ...
                    <p className="text-sm font-black text-slate-800 leading-none">
                      15 <span className="text-[9px] text-slate-400">Juz</span>
                    </p>
                  </div>
                    ...
                    {progressData.map((row, idx) => (
                      <tr key={`${row.kelas}-${row.semester}`} className="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td className="px-4 py-1.5 border-r border-b border-slate-100 bg-emerald-50/5 text-center">
                          <span
                            className={`px-2 py-0.5 rounded-xl text-[8px] font-black uppercase tracking-widest border shadow-sm whitespace-nowrap ${
                              row.hafalanKet === "Tercapai"
                                ? "bg-emerald-50 text-emerald-600 border-emerald-100/50"
                                : row.hafalanKet === "Tidak Tercapai"
                        ...
                        <td className="px-4 py-1.5 border-b border-slate-100 bg-emerald-50/5 text-center">
                          <span
                            className={`px-2 py-0.5 rounded-xl text-[8px] font-black uppercase tracking-widest border shadow-sm whitespace-nowrap ${
                              row.sabaqKet === "Tercapai"
                                ? "bg-emerald-50 text-emerald-600 border-emerald-100/50"
                                : row.sabaqKet === "Tidak Tercapai"
                                  ? "bg-rose-50 text-rose-500 border-rose-100/50"
                                  : "bg-slate-50 text-slate-300 border-slate-100"
                            }`}
                          >
